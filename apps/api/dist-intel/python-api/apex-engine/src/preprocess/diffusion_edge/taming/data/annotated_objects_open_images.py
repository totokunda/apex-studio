...
from src.preprocess.diffusion_edge.taming.data.annotated_objects_dataset import (
...
def load_annotations(
    ...
    with open(descriptor_path) as file:
        ...
        for i, row in tqdm(
            enumerate(reader), total=14620000, desc="Loading OpenImages annotations"
        ):
...
def load_categories(csv_path: Path) -> Dict[str, Category]:
    with open(csv_path) as file:
        ...
        return {
            row[0]: Category(id=row[0], name=row[1], super_category=None)
            for row in reader
        }
...
class AnnotatedObjectsOpenImages(AnnotatedObjectsDataset):
    ...
    def get_path_structure(self) -> Dict[str, str]:
        if self.split not in OPEN_IMAGES_STRUCTURE:
            raise ValueError(
                f"Split [{self.split} does not exist for Open Images data.]"
            )
