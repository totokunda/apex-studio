...
class AnimalPoseImage:
    ...
    def __call__(self, oriImg):
        ...
        if is_model_torchscript(self.det):
            det_result = inference_jit_yolox(
                self.det, oriImg, detect_classes=detect_classes
            )
        else:
            det_start = default_timer()
        ...
        print(
            f"AnimalPose: Pose {((default_timer() - pose_start) * 1000):.2f}ms on {det_result.shape[0]} animals\n"
        )
