...
def main():
    ...
    for subj in range(1, 7):
        ...
        frame_relpaths = [
            f"3dhp/TS{subj}/imageSequence/img_{i + 1:06d}.jpg" for i in valid_frames
        ]
...
def predict_sequence(predict_fn, frames_cpu_gpu, n_frames, camera, viz):
    ...
    for frames_b_cpu, frames_b in spu.progressbar(
        frames_cpu_gpu, total=n_frames, step=FLAGS.batch_size
    ):
