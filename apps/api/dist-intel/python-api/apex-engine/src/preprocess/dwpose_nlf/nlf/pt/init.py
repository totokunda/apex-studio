...
def initialize(args=None, parent_parser=None):
    ...
    if FLAGS.load_path:
        ...
        FLAGS.load_path = util.ensure_absolute_path(
            FLAGS.load_path, FLAGS.checkpoint_dir
        )
...
def get_parser(parent_parser=None):
    ...
    parser.add_argument(
        "--train", action=spu.argparse.BoolAction, help="Train the model."
    )
    parser.add_argument(
        "--render", action=spu.argparse.BoolAction, help="Render current predictions."
    )
    parser.add_argument(
        "--predict", action=spu.argparse.BoolAction, help="Test the model."
    )
    ...
    parser.add_argument(
        "--scale-aug-down",
        type=float,
        help="Scale augmentation in percent.",
        default=25,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--antialias-test", type=int, default=1
    ...
    parser.add_argument(
        "--test-subjects", type=str, default=None, help="Test subjects."
    )
    parser.add_argument(
        "--valid-subjects", type=str, default=None, help="Validation subjects."
    )
    parser.add_argument(
        "--train-subjects", type=str, default=None, help="Training subjects."
    )

    # What to train, validate and test on
    parser.add_argument("--train-on", type=str, default="train", help="Training part.")
    parser.add_argument(
        "--validate-on", type=str, default="valid", help="Validation part."
    )
    ...
    parser.add_argument(
        "--centered-stride", action=spu.argparse.BoolAction, default=True
    )
    ...
    parser.add_argument(
        "--depth-on-2d-heatmap", action=spu.argparse.BoolAction, default=True
    )
    ...
    # Field
    parser.add_argument(
        "--backbone-linking-layer", action=spu.argparse.BoolAction, default=True
    )
    ...
    parser.add_argument(
        "--fix-uncert-factor", action=spu.argparse.BoolAction, default=True
    )
