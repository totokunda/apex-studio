...
class LongCatVideoTransformer3DModel(
    ...
    def __init__(
        ...
        self.lora_dict = {}
        self.active_loras = []
        attention_register._add(
            "bsa", lambda q, k, v, **kwargs: None, allow=True, available=True
        )

    def set_adapters(self, adapter_names, weights):
        non_longcat_adapter_names = [
            adapter_name
            for adapter_name in adapter_names
            if adapter_name not in ["cfg_step_lora", "refinement_lora"]
        ]
    ...
    def load_lora_adapter(
        ...
        **kwargs,
        ...
        if adapter_name == "cfg_step_lora":
            self._load_lora_adapter_for_longcat(
                pretrained_model_name_or_path_or_dict,
                prefix="transformer",
                hotswap=hotswap,
                **kwargs,
    ...
    def _load_lora_adapter_for_longcat(
        self,
        pretrained_model_name_or_path_or_dict,
        prefix="transformer",
        hotswap: bool = False,
        **kwargs,
        ...
        if isinstance(pretrained_model_name_or_path_or_dict, str):
            lora_network_state_dict_loaded = load_file(
