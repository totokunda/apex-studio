...
from diffusers.models.attention_processor import Attention, MochiVaeAttnProcessor2_0
...
class MochiChunkedGroupNorm3D(nn.Module):
    ...
    def __init__(
        ...
        self.norm_layer = nn.GroupNorm(
            num_channels=num_channels, num_groups=num_groups, affine=affine
        )
    ...
    def forward(self, x: torch.Tensor = None) -> torch.Tensor:
        ...
        output = torch.cat(
            [self.norm_layer(chunk) for chunk in x.split(self.chunk_size, dim=0)], dim=0
        )
...
class MochiDownBlock3D(nn.Module):
    ...
    def forward(
        ...
        for i, (resnet, norm, attn) in enumerate(
            zip(self.resnets, self.norms, self.attentions)
        ):
            ...
            if attn is not None:
                ...
                hidden_states = (
                    hidden_states.permute(0, 3, 4, 2, 1).flatten(0, 2).contiguous()
                )
...
class MochiUpBlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Linear(
            in_channels, out_channels * temporal_expansion * spatial_expansion**2
        )
    ...
    def forward(
        ...
        hidden_states = hidden_states.view(
            batch_size, -1, st, sh, sw, num_frames, height, width
        )
        hidden_states = hidden_states.permute(0, 1, 5, 2, 6, 3, 7, 4).contiguous()
        hidden_states = hidden_states.view(
            batch_size, -1, num_frames * st, height * sh, width * sw
        )
...
class FourierFeatures(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        freqs = torch.arange(
            self.start, self.stop, self.step, dtype=inputs.dtype, device=inputs.device
        )
...
class MochiEncoder3D(nn.Module):
    ...
    def __init__(
        ...
        self.block_in = MochiMidBlock3D(
            ...
            add_attention=add_attention_block[0],
        )
        ...
        self.block_out = MochiMidBlock3D(
            ...
            add_attention=add_attention_block[-1],
        )
    ...
    def forward(
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            hidden_states, new_conv_cache["block_in"] = (
                self._gradient_checkpointing_func(
...
class MochiDecoder3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv_in = nn.Conv3d(
            in_channels, block_out_channels[-1], kernel_size=(1, 1, 1)
        )
        self.block_in = MochiMidBlock3D(
            in_channels=block_out_channels[-1],
            num_layers=layers_per_block[-1],
...
class AutoencoderKLMochi(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.spatial_compression_ratio = functools.reduce(
            lambda x, y: x * y, spatial_expansions, 1
        )
        self.temporal_compression_ratio = functools.reduce(
            lambda x, y: x * y, temporal_expansions, 1
        )
    ...
    def _decode(
        ...
        if (
            self.drop_last_temporal_frames
            and dec.size(2) >= self.temporal_compression_ratio
        ):
    ...
    def tiled_decode(
        ...
        for i in range(0, height, tile_latent_stride_height):
            ...
            for j in range(0, width, tile_latent_stride_width):
                ...
                if (
                    self.drop_last_temporal_frames
                    and time.size(2) >= self.temporal_compression_ratio
                ):
