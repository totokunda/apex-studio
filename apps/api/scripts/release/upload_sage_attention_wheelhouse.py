#!/usr/bin/env python3
"""
Upload SageAttention wheelhouse artifacts to Hugging Face Hub.

Uploads:
  apps/api/SageAttention/wheelhouse/sm80   -> sage-attention/sm80
  apps/api/SageAttention/wheelhouse/sm86   -> sage-attention/sm86
  apps/api/SageAttention/wheelhouse/sm89   -> sage-attention/sm89
  apps/api/SageAttention/wheelhouse/sm90   -> sage-attention/sm90
  apps/api/SageAttention/wheelhouse/sm100  -> sage-attention/sm100
  apps/api/SageAttention/wheelhouse/sm120  -> sage-attention/sm120
...
DEFAULT_BASE_DIR = _API_DIR / "SageAttention" / "wheelhouse"
DEFAULT_PATH_PREFIX = "sage-attention"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Upload SageAttention wheelhouse artifacts to Hugging Face Hub."
    )
    parser.add_argument(
        "--repo-id",
    ...
    parser.add_argument(
        "--commit-message",
        default="Upload SageAttention wheelhouse artifacts",
    ...
    if not path_prefix:
        raise SystemExit("--path-prefix must be a non-empty folder name, e.g. sage-attention")
    ...
    uploads: list[tuple[Path, str]] = [
        ...
        (base_dir / "sm86", f"{path_prefix}/sm86"),
        (base_dir / "sm89", f"{path_prefix}/sm89"),
