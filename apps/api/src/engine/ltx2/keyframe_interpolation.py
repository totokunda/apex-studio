...
class LTX2KeyframeInterpolationEngine(LTX2KeyframeVideoBaseEngine):
    """
    LTX2 keyframe interpolation engine that uses the *keyframe-token append* conditioning style.

    - Only supports conditioning via images (no conditioning video).
    - Intended to mimic the ltx-pipelines `KeyframeInterpolationPipeline` conditioning style
      (`VideoConditionByKeyframeIndex`), but within the Apex engine stack.
    """
    ...
    def run(  # noqa: PLR0913
        ...
        return super().run(
            ...
            conditioning_video_strength=1.0,
            conditioning_video_pixel_frame_index=0,
            height=height,
            width=width,
            duration=duration,
            fps=fps,
