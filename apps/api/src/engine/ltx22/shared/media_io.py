...
def load_image_conditioning(
    image: Image.Image, height: int, width: int, dtype: torch.dtype, device: torch.device
) -> torch.Tensor:
    """
...
def load_video_conditioning(
    video: List[torch.Tensor], height: int, width: int,  dtype: torch.dtype, device: torch.device
) -> torch.Tensor:
    """
...
def decode_image(image: Image.Image) -> np.ndarray:
