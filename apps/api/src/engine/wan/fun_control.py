...
class WanFunControlEngine(WanShared):
    ...
    def prepare_mask_latents(
        ...
        if masked_image is not None:
            ...
            for i in range(0, masked_image.shape[0], bs):
                ...
                mask_pixel_values_bs = self.vae_encode(
                    mask_pixel_values_bs, sample_mode="mode"
                )
    ...
    def prepare_control_latents(
        ...
        if control_image is not None:
            ...
            for i in range(0, control_image.shape[0], bs):
                ...
                control_pixel_values_bs = self.vae_encode(
                    control_pixel_values_bs, sample_mode="mode"
                )
    ...
    def get_video_to_video_latent(
        ...
        if ref_image is not None:
            if isinstance(ref_image, str):
                ...
                ref_image = torch.from_numpy(np.array(ref_image))
                ref_image = (
                    ref_image.unsqueeze(0).permute([3, 0, 1, 2]).unsqueeze(0) / 255
                )
            else:
                ref_image = torch.from_numpy(np.array(ref_image))
                ref_image = (
    ...
    def run(
        ...
        **kwargs,
        ...
        safe_emit_progress(
            progress_callback, 0.03, "Preparing start/end images to video latents"
        )
        video, mask_video, _ = self.get_image_to_video_latent(
            start_image, end_image, video_length=num_frames, sample_size=sample_size
        )
        ...
        if control_camera_poses is not None:
            ...
            safe_emit_progress(
                progress_callback, 0.05, "Preparing camera control video"
            )
        ...
        if start_image is not None:
            ...
            start_image = self.image_processor.preprocess(
                rearrange(start_image, "b c f h w -> (b f) c h w"),
                height=height,
                width=width,
            )
            start_image = start_image.to(dtype=torch.float32)
            start_image = rearrange(
                start_image, "(b f) c h w -> b c f h w", f=video_length
            )
        ...
        if self.transformer_config.get("add_ref_conv", False):
            if ref_image is not None:
                safe_emit_progress(
                    progress_callback, 0.42, "Preprocessing reference image latents"
                )
                ...
                ref_image = rearrange(
                    ref_image, "(b f) c h w -> b c f h w", f=video_length
                )
        ...
        if init_video is not None:
            ...
            control_latents_input = (
                y
                if control_latents_input is None
                else torch.cat([control_latents_input, y], dim=1)
            )
        else:
            start_image_latentes_conv_in_input = (start_image_latentes_conv_in).to(
                device, weight_dtype
            )
            control_latents_input = (
                ...
                if control_latents_input is None
                    ...
                    [control_latents_input, start_image_latentes_conv_in_input], dim=1
                )
            )
        ...
        latents = self.denoise(
            ...
            transformer_kwargs=dict(
                ...
                encoder_hidden_states_full_ref=(
                    full_ref.to(transformer_dtype) if full_ref is not None else None
                ),
