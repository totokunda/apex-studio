...
class DDPM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.weighting_loss = (
            cfg.get("weighting_loss", False) if cfg is not None else False
        )
        ...
        if self.objective == "pred_noise":
            lvlb_weights = self.betas**2 / (
                2
                * (self.posterior_variance + 1e-5)
                * alphas
                * (1 - self.alphas_cumprod)
            )
        elif self.objective == "pred_x0":
            lvlb_weights = 0.5 * torch.sqrt(alphas_cumprod) / (2.0 * 1 - alphas_cumprod)
    ...
    def p_sample_loop(self, shape, mask, up_scale=1, unnormalize=True):
        ...
        img = F.interpolate(
            img, scale_factor=up_scale, mode="bilinear", align_corners=True
        )
    ...
    def interpolate(self, x1, x2, mask, t=None, lam=0.5):
        ...
        for i in tqdm(
            ...
            img = self.p_sample(
                img, mask, torch.full((b,), i, device=device, dtype=torch.long)
            )
        return img

    def get_input(
        self, batch, return_first_stage_outputs=False, return_original_cond=False
    ):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # t = t.round() * eps
        # t[t < eps] = eps
        t = torch.rand(x.shape[0], device=x.device) * (1.0 - eps) + eps
    ...
    def q_sample2(self, x_start, t, noise=None):
        ...
        x = x_start.expand(nt, b, c, h, w).transpose(1, 0) * param_x.reshape(
            b, nt, 1, 1, 1
        ).repeat(1, 1, c, h, w)
        param_noise = self.sqrt_one_minus_alphas_cumprod.repeat(b, 1).gather(-1, t)
        n = noise.expand(nt, b, c, h, w).transpose(1, 0) * param_noise.reshape(
            b, nt, 1, 1, 1
        ).repeat(1, 1, c, h, w)
    ...
    def q_sample3(self, x_start, t, C):
        ...
        x_noisy = (
            x_start.expand(nt, b, c, h, w).transpose(1, 0)
    ...
    def p_losses(self, x_start, t, *args, **kwargs):
        if self.start_dist == "normal":
            ...
            noise = 2 * torch.rand_like(x_start) - 1.0
        else:
        ...
        loss_simple += simple_weight1 * self.get_loss(C_pred, target1, mean=False).mean(
            [1, 2, 3]
        ) + simple_weight2 * self.get_loss(noise_pred, target2, mean=False).mean(
            [1, 2, 3]
        )
        if self.use_l1:
            loss_simple += simple_weight1 * (C_pred - target1).abs().mean(
                [1, 2, 3]
    ...
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        ...
        if denoise:
            ...
            time_steps = torch.cat(
                (time_steps[:-1], torch.tensor([step - eps]), torch.tensor([eps])),
                dim=0,
            )
        ...
        if self.start_dist == "normal":
            ...
            img = 2 * torch.rand(shape, device=device) - 1.0
        else:
            raise NotImplementedError(f"{self.start_dist} is not supported !")
        img = F.interpolate(
            img, scale_factor=up_scale, mode="bilinear", align_corners=True
...
class LatentDiffusion(DDPM):
    ...
    def q_sample3(self, x_start, t, K, C):
        ...
        x_noisy = (
            ...
            + K_
            / 2
            * (t.reshape(b, nt, 1, 1, 1).repeat(1, 1, c, h, w) / self.num_timesteps)
            ** 2
            + C_ * t.reshape(b, nt, 1, 1, 1).repeat(1, 1, c, h, w) / self.num_timesteps
        )
        return x_noisy  # (b, nt, c, h, w)
    ...
    def Dice_Loss(self, pred, label):
        ...
        intersecion = pred_flat * label_flat
        unionsection = (
            pred_flat.pow(2).sum(dim=-1) + label_flat.pow(2).sum(dim=-1) + smooth
        )
    ...
    def p_losses(self, x_start, t, *args, **kwargs):
        ...
        if self.cfg.model_name == "cond_unet8":
            loss_simple += 0.05 * (
                self.Dice_Loss(e1, (kwargs["edge"] + 1) / 2)
                + self.Dice_Loss(e2, (kwargs["edge"] + 1) / 2)
            )
        elif self.cfg.model_name == "cond_unet13":
            loss_simple += 0.5 * (
                simple_weight1
                * self.get_loss(aux_C, target1, mean=False).mean([1, 2, 3])
        ...
        if self.cfg.get("use_disloss", False):
            with torch.no_grad():
                ...
                edge_rec = torch.clamp(edge_rec, min=0.0, max=1.0)  # B, 1, 320, 320
            loss_tmp = (
                self.cross_entropy_loss_RCF(edge_rec, (kwargs["edge"] + 1) / 2)
                * rec_weight
            )  # B, 1
    ...
    def sample(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True):
        ...
        if self.cfg.model_name == "cond_unet8" or self.cfg.model_name == "cond_unet13":
            z, aux_out = self.sample_fn(
                (
                    batch_size,
                    channels,
                    image_size[0] // down_ratio,
                    image_size[1] // down_ratio,
                ),
                up_scale=up_scale,
                unnormalize=False,
                cond=cond,
        ...
        if self.scale_by_std:
            z = 1.0 / self.scale_factor * z.detach()
            if self.cfg.model_name == "cond_unet13":
                aux_out = 1.0 / self.scale_factor * aux_out.detach()
    ...
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        ...
        img_aux = F.interpolate(
            img.clone(), scale_factor=up_scale, mode="bilinear", align_corners=True
        )
        # img_aux = img.clone()
        # K = -1 * torch.ones_like(img)
        cur_time = torch.ones((batch,), device=device)
        for i, time_step in enumerate(time_steps):
            ...
            C, noise = pred[:2]
            if (
                self.cfg.model_name == "cond_unet8"
                or self.cfg.model_name == "cond_unet13"
            ):
                aux_out = pred[-1]
            else:
                aux_out = None
