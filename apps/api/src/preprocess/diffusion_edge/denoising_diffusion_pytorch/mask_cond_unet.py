...
from src.preprocess.diffusion_edge.denoising_diffusion_pytorch.efficientnet import (
    efficientnet_b7,
    EfficientNet_B7_Weights,
)
from src.preprocess.diffusion_edge.denoising_diffusion_pytorch.resnet import (
    resnet101,
    ResNet101_Weights,
)
from src.preprocess.diffusion_edge.denoising_diffusion_pytorch.swin_transformer import (
    swin_b,
    Swin_B_Weights,
)
from src.preprocess.diffusion_edge.denoising_diffusion_pytorch.vgg import (
    vgg16,
    VGG16_Weights,
)
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.zeros(x.shape[0], x.shape[1], x.shape[2], device=x.device).to(
            torch.bool
        )
        batch = mask.shape[0]
...
class BasicAttetnionLayer(nn.Module):
    ...
    def forward(self, x1, x2):  # x1 for q (conditional input), x2 for k,v
        ...
        shortcut = x2 + self.concat_conv(
            torch.cat(
                [
                    F.interpolate(
                        x1, size=(H2, W2), mode="bilinear", align_corners=True
                    ),
                    x2,
                ],
                dim=1,
        ...
        pad_l = pad_t = 0
        pad_r = (self.window_size1[1] - W1 % self.window_size1[1]) % self.window_size1[
            1
        ]
        pad_b = (self.window_size1[0] - H1 % self.window_size1[0]) % self.window_size1[
            0
        ]
        ...
        pad_l = pad_t = 0
        pad_r = (self.window_size2[1] - W2 % self.window_size2[1]) % self.window_size2[
            1
        ]
        pad_b = (self.window_size2[0] - H2 % self.window_size2[0]) % self.window_size2[
            0
        ]
        ...
        qg = (
            self.q_lin(qg)
            .reshape(B, num_window_q, self.nhead, C1 // self.nhead)
            .permute(0, 2, 1, 3)
            .contiguous()
        )
        kg2 = (
            self.k_lin(kg)
            .reshape(B, num_window_k, self.nhead, C1 // self.nhead)
            .permute(0, 2, 1, 3)
            .contiguous()
        )
        vg = (
        ...
        qg = qg.transpose(1, 2).reshape(
            B, C1, H1p // self.window_size1[0], W1p // self.window_size1[1]
        )
...
class ResnetDownsampleBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None):
        ...
        return h + self.res_conv(F.interpolate(x, size=h.shape[-2:], mode="bilinear"))
...
class ConditionEncoder(nn.Module):
    def __init__(self, down_dim_mults=(2, 4, 8), dim=64, in_dim=1, out_dim=64):
        ...
        for i_level in range(self.num_resolutions):
            ...
            self.downs.append(ResnetDownsampleBlock(dim=block_in, dim_out=block_out))
...
class Unet(nn.Module):
        ...
        **kwargs,
        ...
        if cfg.cond_net == "effnet":
            ...
            if cfg.get("without_pretrain", False):
                ...
                swin_b_model.load_state_dict(
                    torch.load(custom_torch_download(filename="swin_b-68c6b09e.pth")),
                    strict=False,
                )
        ...
        if self.cond_pe:
            self.cond_pos_embedding = nn.Sequential(
                PositionEmbeddingLearned(
                    feature_size=cfg.cond_feature_size,
                    num_pos_feats=cfg.num_pos_feats // 2,
                ),
        ...
        feature_size_list = [
            [int(input_size[0] / 2**k), int(input_size[1] / 2**k)]
            for k in range(len(dim_mults))
        ]
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.relation_layers_up.append(
                RelationNet(
            ...
            self.relation_layers_up2.append(
                RelationNet(
                    in_channel1=dims_rev[ind + 1],
                    in_channel2=dims_rev[ind],
                    nhead=8,
    ...
    def forward(self, x, time, mask, x_self_cond=None, **kwargs):
        ...
        x = self.init_conv(
            torch.cat(
                [x, F.interpolate(hm[0], size=x.shape[-2:], mode="bilinear")], dim=1
            )
        )
        r = x.clone()
        ...
        # hm = []
        # hm2 = []
        for i, ((block1, block2, attn, downsample), relation_layer) in enumerate(
            zip(self.downs, self.relation_layers_down)
        ):
            x = block1(x, t)
            h.append(x)
            h2.append(x.clone())
        ...
        x = x1
        for (block1, block2, attn, upsample), relation_layer in zip(
            self.ups, self.relation_layers_up
        ):
        ...
        x = x2
        for (block1, block2, attn, upsample), relation_layer in zip(
            self.ups2, self.relation_layers_up2
        ):
