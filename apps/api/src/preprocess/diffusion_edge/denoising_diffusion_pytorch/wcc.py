...
from src.preprocess.diffusion_edge.denoising_diffusion_pytorch.quantization import (
    weight_quantize_fn,
    act_quantize_fn,
)
...
class WCC(nn.Conv1d):
    def __init__(
        ...
        super(WCC, self).__init__(
            in_channels, out_channels, 1, stride, padding, dilation, groups, bias
        )
        self.layer_type = "WCC"
        self.bit_w = bit_w
        self.bit_a = bit_a
        ...
        dec_filters, rec_filters = wavelet.create_wavelet_filter(
            wave=self.wt_type, in_size=in_channels, out_size=out_channels
        )
    ...
    def decompress(self, topk, ids, shape):
        ...
        x = torch.zeros(
            size=(b, self.out_channels, h * w), requires_grad=True, device=topk.device
        )
    ...
    def change_wt_params(self, compress_rate, levels, wt_type="db1"):
        ...
        self.levels = levels
        dec_filters, rec_filters = wavelet.create_wavelet_filter(
            wave=self.wt_type, in_size=self.in_channels, out_size=self.out_channels
        )
        self.wt_filters = nn.Parameter(dec_filters, requires_grad=False)
        self.iwt_filters = nn.Parameter(rec_filters, requires_grad=False)
        self.wt = wavelet.get_transform(self.wt_filters, self.in_channels, levels)
        self.iwt = wavelet.get_inverse_transform(
            self.iwt_filters, self.out_channels, levels
        )
