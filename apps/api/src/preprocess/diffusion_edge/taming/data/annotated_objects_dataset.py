...
from src.preprocess.diffusion_edge.taming.data.conditional_builder.objects_bbox import (
    ObjectsBoundingBoxConditionalBuilder,
)
from src.preprocess.diffusion_edge.taming.data.conditional_builder.objects_center_points import (
    ObjectsCenterPointsConditionalBuilder,
)
from src.preprocess.diffusion_edge.taming.data.conditional_builder.utils import (
    load_object_from_string,
)
from src.preprocess.diffusion_edge.taming.data.helper_types import (
    BoundingBox,
    CropMethodType,
    Image,
    Annotation,
...
from src.preprocess.diffusion_edge.taming.data.image_transforms import (
...
class AnnotatedObjectsDataset(Dataset):
    ...
    def no_classes(self) -> int:
        return (
            self.no_object_classes if self.no_object_classes else len(self.categories)
        )
    ...
    def filter_categories(self) -> None:
        if self.category_allow_list:
            self.categories = {
                id_: cat
                for id_, cat in self.categories.items()
                if cat.name in self.category_allow_list
            }
        if self.category_mapping:
            self.categories = {
                id_: cat
                for id_, cat in self.categories.items()
                if cat.id not in self.category_mapping
            }
    ...
    def __getitem__(self, n: int) -> Dict[str, Any]:
        ...
        if "image" in self.keys:
            ...
            sample["crop_bbox"], sample["flipped"], sample["image"] = (
