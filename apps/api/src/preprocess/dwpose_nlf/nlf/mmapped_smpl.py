    ...
    def __call__(
        ...
        *,
        ...
        if rel_rotmats is not None:
            ...
            rel_rotmats = rotvec2mat(
                np.reshape(pose_rotvecs, (batch_size, self.num_joints, 3))
            )
        ...
        for i_joint in range(1, self.num_joints):
            ...
            glob_positions.append(
                ...
                + np.einsum(
                    "bCc,bc->bC", glob_rotmats[i_parent], j[:, i_joint] - j[:, i_parent]
                )
            )
        ...
        pose_feature = np.reshape(
            rel_rotmats[:, 1:], [-1, (self.num_joints - 1) * 3 * 3]
        )
        v_posed = (
            self.v_template
            + np.einsum(
