...
def main():
    ...
    for i_subject in (9, 11):
        for activity_name, camera_id in itertools.product(
            ...
            video_relpath, frame_relpaths, bboxes, camera, world_coords_gt = (
                get_sequence(i_subject, activity_name, camera_id)
            )
            ...
            coords3d_pred_world = predict_sequence(
                predict_fn, frames_cpu_gpu, box_batches, camera, viz
            )
    ...
    np.savez(
        FLAGS.output_path,
        image_path=np.concatenate(image_relpaths_all, axis=0),
...
def predict_sequence(predict_fn, frames_cpu_gpu, box_batches, camera, viz):
    predict_fn = functools.partial(
        ...
        extrinsic_matrix=torch.from_numpy(
            camera.get_extrinsic_matrix()[np.newaxis]
        ).cuda(),
        distortion_coeffs=torch.from_numpy(
            camera.get_distortion_coeffs()[np.newaxis]
        ).cuda(),
...
def get_sequence(i_subject, activity_name, i_camera):
    ...
    bbox_path = (
        f"{DATA_ROOT}/h36m/S{i_subject}/BBoxes/{activity_name}.{camera_name}.npy"
    )
    ...
    world_coords = coords_raw.reshape([coords_raw.shape[0], -1, 3])[
        :, i_relevant_joints
    ]
