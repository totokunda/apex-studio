...
def image_files(
    image_paths,
    extra_data=None,
    internal_queue_size=None,
    batch_size=64,
    prefetch_gpu=1,
    tee_cpu=False,
    frame_preproc_fn=None,
    ...
    return image_dataset_from_queue(
        images_from_paths_gen,
        args=(image_paths, downscale_factor),
        imshape=[height, width],
        extra_data=extra_data,
        internal_queue_size=internal_queue_size,
...
def video_file(
    ...
    return image_dataset_from_queue(
        sliced_reader,
        args=(video_path, video_slice, downscale_factor),
        imshape=[height, width],
        extra_data=extra_data,
        internal_queue_size=internal_queue_size,
        batch_size=batch_size,
...
def video_files(
    ...
    return image_dataset_from_queue(
        concat_frame_gen,
        args=(video_paths, video_slice, downscale_factor),
        imshape=[height, width],
        extra_data=extra_data,
        internal_queue_size=internal_queue_size,
        batch_size=batch_size,
...
def youtube(
    ...
    return image_dataset_from_queue(
        frames_of_youtube_stream,
        args=(best_stream.url,),
        imshape=[height, width],
        extra_data=extra_data,
        internal_queue_size=internal_queue_size,
        batch_size=batch_size,
...
def sliced_reader(path, video_slice, downscale_factor):
    ...
    with imageio.get_reader(path, "ffmpeg", output_params=output_params) as video:
        yield from itertools.islice(
            video, video_slice.start, video_slice.stop, video_slice.step
        )
...
def interleaved_frame_gen(video_paths, video_slice, downscale_factor):
    video_readers = [
        sliced_reader(p, video_slice, downscale_factor) for p in video_paths
    ]
...
def image_dataset_from_queue(
    ...
    ds = tf.data.Dataset.from_generator(
        lambda: frames, output_signature=output_signature
    )
