...
class PersonDetector(nn.Module):
        ...
        if model_path.lower().endswith(".onnx"):
            ...
            try:
                ...
                raise ModuleNotFoundError(
                    "onnxruntime is required to run PersonDetector with an .onnx model. "
                    "Install onnxruntime (or onnxruntime-gpu) or provide a TorchScript model."
                ) from e
            ...
            # e.g. "tensor(float)" / "tensor(float16)"
            self._ort_input_type = self._ort_session.get_inputs()[0].type
            self._ort_output_names = [o.name for o in self._ort_session.get_outputs()]
            self._backend = "onnx"
        else:
            # TorchScript backend (kept for compatibility with older pipelines).
            if model_path.lower().endswith(".pt"):
                raise ValueError(
                    "PersonDetector expects a YOLO model exported as ONNX (.onnx) or TorchScript "
                    "(.torchscript/.pt2). A raw Ultralytics weights file (.pt) is not supported here."
                )
    ...
    def call_model(self, images):
        if self._backend == "onnx":
            ...
            np_dtype = np.float16 if "float16" in self._ort_input_type else np.float32
            ort_input = (
                images.detach()
        ...
        # YOLO exports commonly produce either [B, 84, N] or [B, N, 84]
        if preds.ndim == 3 and preds.shape[-1] != 84 and preds.shape[1] == 84:
            preds = preds.permute(0, 2, 1)  # [batch, n_boxes, 84]
