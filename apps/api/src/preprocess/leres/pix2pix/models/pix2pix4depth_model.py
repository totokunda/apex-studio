...
class Pix2Pix4DepthModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument(
                "--lambda_L1", type=float, default=1000, help="weight for L1 loss"
            )
        return parser
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(
                self.netG.parameters(), lr=1e-4, betas=(opt.beta1, 0.999)
            )
            self.optimizer_D = torch.optim.Adam(
                self.netD.parameters(), lr=2e-06, betas=(opt.beta1, 0.999)
            )
    ...
    def set_input_train(self, input):
        ...
        self.outer = torch.nn.functional.interpolate(
            self.outer, (1024, 1024), mode="bilinear", align_corners=False
        )

        self.inner = input["data_inner"].to(self.device)
        self.inner = torch.nn.functional.interpolate(
            self.inner, (1024, 1024), mode="bilinear", align_corners=False
        )
        ...
        if self.isTrain:
            ...
            self.gtfake = torch.nn.functional.interpolate(
                self.gtfake, (1024, 1024), mode="bilinear", align_corners=False
            )
