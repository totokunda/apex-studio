...
from custom_mesh_graphormer.utils.tsv_file_ops import (
    load_linelist_file,
    load_from_yaml_file,
    find_file_path_in_yaml,
)
from custom_mesh_graphormer.utils.image_ops import (
    img_from_base64,
    crop,
    flip_img,
    flip_pose,
...
class HandMeshTSVDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        meta_data["pose"] = torch.from_numpy(
            self.pose_processing(pose, rot, flip)
        ).float()
...
class HandMeshTSVYamlDataset(HandMeshTSVDataset):
    ...
    def __init__(
        self, args, yaml_file, is_train=True, cv2_output=False, scale_factor=1
    ):
