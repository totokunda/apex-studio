...
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.mps_wrapper import (
    sparse_to_dense,
)
...
class MANO(nn.Module):
    def __init__(self):
        ...
        self.fingertip_vertex_idx = [
            ...
            556,
            673,
        ]  # mesh vertex idx (right hand)
        thumbtip_onehot = np.array(
            [1 if i == 745 else 0 for i in range(self.joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        indextip_onehot = np.array(
            [1 if i == 317 else 0 for i in range(self.joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        middletip_onehot = np.array(
            [1 if i == 445 else 0 for i in range(self.joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        ringtip_onehot = np.array(
            [1 if i == 556 else 0 for i in range(self.joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        pinkytip_onehot = np.array(
            [1 if i == 673 else 0 for i in range(self.joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
    ...
    def get_layer(self):
        return ManoLayer(
            mano_root=osp.join(self.mano_dir), flat_hand_mean=False, use_pca=False
