...
class SMPL(nn.Module):
    ...
    def __init__(self, gender="neutral"):
        ...
        self.register_buffer(
            "J_regressor", torch.sparse_coo_tensor(i, v, J_regressor_shape).to_dense()
        )
        ...
        J_regressor_extra = torch.from_numpy(
            np.load(cfg.JOINT_REGRESSOR_TRAIN_EXTRA)
        ).float()
        ...
        J_regressor_h36m_correct = torch.from_numpy(
            np.load(cfg.JOINT_REGRESSOR_H36M_correct)
        ).float()
...
class Mesh(object):
    ...
    def __init__(
        self,
        filename=cfg.SMPL_sampling_matrix,
        num_downsampling=1,
        nsize=1,
