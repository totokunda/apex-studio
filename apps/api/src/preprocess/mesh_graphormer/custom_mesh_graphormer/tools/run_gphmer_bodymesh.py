...
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling.bert import (
    Graphormer_Body_Network as Graphormer_Network,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling._smpl import (
    SMPL,
    Mesh,
)
...
def keypoint_3d_loss(
    criterion_keypoints, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d, device
):
    """
...
def run(
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        loss_vertices = (
            ...
            * vertices_loss(
            ...
            + args.vloss_w_full
            * vertices_loss(
                criterion_vertices, pred_vertices, gt_vertices, has_smpl, args.device
            )
        )
        ...
        if iteration % iters_per_epoch == 0:
            ...
            logger.info(
                ...
                + "  mPVE: {:6.2f}, mPJPE: {:6.2f}, PAmPJPE: {:6.2f}, Data Count: {:6.2f}".format(
                    1000 * val_mPVE, 1000 * val_mPJPE, 1000 * val_PAmPJPE, val_count
                )
            )
    ...
    logger.info(
        " Best Results:"
        + "  mPVE: {:6.2f}, mPJPE: {:6.2f}, PAmPJPE: {:6.2f}, at epoch {:6.2f}".format(
            1000 * log_eval_metrics.mPVE,
...
def run_eval_general(args, val_dataloader, Graphormer_model, smpl, mesh_sampler):
    ...
    logger.info(
        ...
        + "  mPVE: {:6.2f}, mPJPE: {:6.2f}, PAmPJPE: {:6.2f} ".format(
            1000 * val_mPVE, 1000 * val_mPJPE, 1000 * val_PAmPJPE
        )
    )
...
def run_validate(
    ...
    with torch.no_grad():
        ...
        for i, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            gt_smpl_3d_joints = smpl.get_h36m_joints(gt_vertices)
            gt_smpl_3d_pelvis = gt_smpl_3d_joints[
                :, cfg.H36M_J17_NAME.index("Pelvis"), :
            ]
            gt_vertices_sub2 = gt_vertices_sub2 - gt_smpl_3d_pelvis[:, None, :]
            gt_vertices = gt_vertices - gt_smpl_3d_pelvis[:, None, :]
            ...
            (
                ...
                pred_vertices_sub,
                pred_vertices,
            ) = Graphormer_model(images, smpl, mesh_sampler)

            # obtain 3d joints from full mesh
            pred_3d_joints_from_smpl = smpl.get_h36m_joints(pred_vertices)

            pred_3d_pelvis = pred_3d_joints_from_smpl[
                :, cfg.H36M_J17_NAME.index("Pelvis"), :
            ]
            ...
            pred_3d_joints_from_smpl = (
                pred_3d_joints_from_smpl - pred_3d_pelvis[:, None, :]
            )
            pred_vertices = pred_vertices - pred_3d_pelvis[:, None, :]

            # measure errors
            error_vertices = mean_per_vertex_error(pred_vertices, gt_vertices, has_smpl)
            error_joints = mean_per_joint_position_error(
                pred_3d_joints_from_smpl, gt_3d_joints, has_3d_joints
            )
...
def main(args):
    ...
    if args.run_eval_only == True:
        ...
        run(
            args, train_dataloader, val_dataloader, _model, smpl, mesh_sampler, renderer
        )
