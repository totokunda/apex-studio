...
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling.bert import (
    Graphormer_Hand_Network as Graphormer_Network,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling._mano import (
    MANO,
    Mesh,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling.hrnet.hrnet_cls_net_gridfeat import (
    get_cls_net_gridfeat,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling.hrnet.config import (
    config as hrnet_config,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.modeling.hrnet.config import (
...
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.datasets.build import (
    make_hand_data_loader,
)

from src.preprocess.mesh_graphormer.custom_mesh_graphormer.utils.logger import (
    setup_logger,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.utils.comm import (
    synchronize,
    is_main_process,
    get_rank,
    get_world_size,
...
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.utils.metric_logger import (
    AverageMeter,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.utils.renderer import (
    Renderer,
    visualize_reconstruction,
    visualize_reconstruction_test,
    visualize_reconstruction_no_text,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.utils.metric_pampjpe import (
    reconstruction_error,
)
from src.preprocess.mesh_graphormer.custom_mesh_graphormer.utils.geometric_layers import (
...
def save_checkpoint(model, args, epoch, iteration, num_trial=10):
    ...
    for i in range(num_trial):
        try:
            ...
            torch.save(
                model_to_save.state_dict(), op.join(checkpoint_dir, "state_dict.bin")
            )
...
def keypoint_2d_loss(
    criterion_keypoints, pred_keypoints_2d, gt_keypoints_2d, has_pose_2d
):
    """
    ...
    loss = (
        conf * criterion_keypoints(pred_keypoints_2d, gt_keypoints_2d[:, :, :-1])
    ).mean()
    return loss


def keypoint_3d_loss(
    criterion_keypoints, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d
):
    """
...
def run(args, train_dataloader, Graphormer_model, mano_model, renderer, mesh_sampler):
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        gt_3d_joints_with_tag = torch.ones((batch_size, gt_3d_joints.shape[1], 4)).to(
            device
        )
        ...
        pred_camera, pred_3d_joints, pred_vertices_sub, pred_vertices = (
            Graphormer_model(
                images, mano_model, mesh_sampler, meta_masks=meta_masks, is_train=True
            )
        )
        ...
        pred_2d_joints = orthographic_projection(
            pred_3d_joints.contiguous(), pred_camera.contiguous()
        )

        # compute 3d joint loss  (where the joints are directly output from transformer)
        loss_3d_joints = keypoint_3d_loss(
            criterion_keypoints, pred_3d_joints, gt_3d_joints_with_tag, has_3d_joints
        )

        # compute 3d vertex loss
        loss_vertices = args.vloss_w_sub * vertices_loss(
            criterion_vertices, pred_vertices_sub, gt_vertices_sub, has_mesh
        ) + args.vloss_w_full * vertices_loss(
            criterion_vertices, pred_vertices, gt_vertices, has_mesh
        )
        ...
        loss_2d_joints = keypoint_2d_loss(
            criterion_2d_keypoints, pred_2d_joints, gt_2d_joints, has_2d_joints
        ) + keypoint_2d_loss(
            criterion_2d_keypoints,
            pred_2d_joints_from_mesh,
            gt_2d_joints,
            has_2d_joints,
        )
        ...
        if iteration % iters_per_epoch == 0:
            if epoch % 10 == 0:
                checkpoint_dir = save_checkpoint(
                    Graphormer_model, args, epoch, iteration
                )
...
def run_eval_and_save(
    args, split, val_dataloader, Graphormer_model, mano_model, renderer, mesh_sampler
):
...
def run_aml_inference_hand_mesh(
    args,
    val_loader,
    Graphormer_model,
    criterion,
    ...
    with torch.no_grad():
        for i, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            # forward-pass
            pred_camera, pred_3d_joints, pred_vertices_sub, pred_vertices = (
                Graphormer_model(images, mano_model, mesh_sampler)
            )
...
def visualize_mesh(
    renderer, images, gt_keypoints_2d, pred_vertices, pred_camera, pred_keypoints_2d
):
    """Tensorboard logging."""
    ...
    for i in range(min(batch_size, 10)):
        ...
        rend_img = visualize_reconstruction(
            img, 224, gt_keypoints_2d_, vertices, pred_keypoints_2d_, cam, renderer
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--num_workers", default=4, type=int, help="Workers in dataloader."
    )
    parser.add_argument(
        "--img_scale_factor", default=1, type=int, help="adjust image resolution."
    )
    #########################################################
    ...
    parser.add_argument(
        "--model_name_or_path",
        default="src/modeling/bert/bert-base-uncased/",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--lr", "--learning_rate", default=1e-4, type=float, help="The initial lr."
    )
    parser.add_argument(
        "--num_train_epochs",
        default=200,
    ...
    parser.add_argument(
        "--drop_out", default=0.1, type=float, help="Drop out ratio in BERT."
    )
    #########################################################
    # Model architectures
    #########################################################
