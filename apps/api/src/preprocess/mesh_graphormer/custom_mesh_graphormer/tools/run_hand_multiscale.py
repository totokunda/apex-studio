...
def multiscale_fusion(output_dir):
    ...
    for s in scale:
        for r in rotations:
            ...
            pa_joint_error, pa_joint_array, _ = get_alignMesh(
                joints_array, ref_joints_array, reduction=None
            )
            pa_vertices_error, pa_vertices_array, _ = get_alignMesh(
                vertices_array, ref_vertices_array, reduction=None
            )
            print("--------------------------")
    ...
    pa_joint_error, pa_joint_array, _ = get_alignMesh(
        overall_joints_array, ref_joints_array, reduction=None
    )
    pa_vertices_error, pa_vertices_array, _ = get_alignMesh(
        overall_vertices_array, ref_vertices_array, reduction=None
    )
    print("--------------------------")
    print("overall:")
    print("PAMPJPE:", 1000 * np.mean(pa_joint_error))
