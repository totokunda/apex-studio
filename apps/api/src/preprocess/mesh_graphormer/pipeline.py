...
args = Namespace(
    ...
    model_name_or_path=str(
        Path(custom_mesh_graphormer.__file__).parent / "modeling/bert/bert-base-uncased"
    ),
    ...
    device="cpu",
    seed=88,
    hrnet_checkpoint=custom_hf_download(
...
class MeshGraphormerMediapipe(Preprocessor):
    ...
    def get_rays(self, W, H, fx, fy, cx, cy, c2w_t, center_pixels):  # rot = I
        ...
        rays_d = (rays_d / np.linalg.norm(rays_d, axis=-1, keepdims=True)).reshape(
            -1, 3
        )
    ...
    def run_inference(self, img, Graphormer_model, mano, mesh_sampler, scale, crop_len):
        ...
        with torch.no_grad():
            ...
            (
                ...
                hidden_states,
                att,
            ) = Graphormer_model(batch_imgs, mano, mesh_sampler)
            ...
            pred_2d_joints_img_space = (
                (pred_3d_joints_camera / z_3d_dist[:, None])
                * np.array((focal_length, focal_length, 1))
            )[:, :2] + np.array((W / 2, H / 2))

            rays_o, rays_d = self.get_rays(
                W, H, focal_length, focal_length, W / 2, H / 2, camera_t, True
            )
            coords = (
                np.array(list(np.ndindex(H, W)))
                .reshape(H, W, -1)
                .transpose(1, 0, 2)
                .reshape(-1, 2)
            )
            intersector = RayMeshIntersector(mesh)
            points, index_ray, _ = intersector.intersects_location(
                rays_o, rays_d, multiple_hits=False
            )
            ...
            discriminator = (
                np.sum(mesh.face_normals[tri_index] * rays_d[index_ray], axis=-1) <= 0
            )
            points = points[
                discriminator
            ]  # ray intesects in interior faces, discard them
            ...
            depthmap[pixel_ray[:, 0], pixel_ray[:, 1]] = 1.0 - (
                0.8 * (depth - minval) / (maxval - minval)
            )
            depthmap *= 255
    ...
    def get_depth(self, np_image, padding):
        ...
        for idx in range(len(hand_landmarks_list)):
            ...
            cropped = padded_image[
                crop_y_min : crop_y_max + 1, crop_x_min : crop_x_max + 1
            ]
            ...
            if crop_len < 224:
                graphormer_input = cv2.resize(
                    cropped, (224, 224), interpolation=cv2.INTER_CUBIC
                )
            else:
                graphormer_input = cv2.resize(
            ...
            cropped_depthmap, pred_2d_keypoints = self.run_inference(
                graphormer_input.astype(np.uint8),
                self._model,
                self.mano_model,
                self.mesh_sampler,
                scale,
            ...
            if len(nonzero_y) == 0 or len(nonzero_x) == 0:
                depth_failure = True
                break
            padded_depthmap[crop_y_min + nonzero_y, crop_x_min + nonzero_x] = (
                resized_cropped_depthmap[nonzero_y, nonzero_x]
            )
    ...
    def eval_mpjpe(self, sample, info):
        ...
        for i in range(len(crop_boxes)):  # box in crop_boxes:
            ...
            generated_keypoint = self.get_keypoints(
                graphormer_input.astype(np.uint8),
                self._model,
                self.mano_model,
                self.mesh_sampler,
            ...
            pjpe += np.sum(
                np.sqrt(
                    np.sum(
                        (
                            (generated_keypoint - groundtruth_2d_keypoints[i]) ** 2
                        ).numpy(),
                        axis=1,
                    )
                )
