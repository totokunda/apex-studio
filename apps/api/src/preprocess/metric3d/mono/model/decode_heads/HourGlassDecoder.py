...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3):
        ...
        if kernel_size == 3:
            ...
            self.conv = nn.Conv2d(
                int(in_channels), int(out_channels), 1, padding=0, stride=1
            )
...
class DecoderFeature(nn.Module):
    def __init__(self, feat_channels, num_ch_dec=[64, 64, 128, 256]):
        ...
        self.upconv_3_0 = ConvBlock(
            self.feat_channels[3], self.num_ch_dec[3], kernel_size=1
        )
        ...
        self.upconv_2_0 = ConvBlock(
            self.num_ch_dec[3], self.num_ch_dec[2], kernel_size=3
        )
        ...
        self.upconv_1_0 = ConvBlock(
            self.num_ch_dec[2], self.num_ch_dec[1], kernel_size=3
        )
...
class HourglassDecoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.decoder_channels = (
            cfg.model.decode_head.decoder_channel
        )  # [64, 64, 128, 256]
    ...
    def forward(self, features_mono, **kwargs):
        ...
        depth_pred_mono = self.upsample(
            depth_pred_2, scale_factor=4
        ) + 1e-1 * self.conv_up_2(
