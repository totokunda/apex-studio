    ...
    def forward(
        ...
        **kwargs,
        ...
        if task != "depth":
            ...
            assert (
                len(attn_splits_list) == len(prop_radius_list) == self.num_scales == 1
            )
        ...
        for scale_idx in range(self.num_scales):
            ...
            if task == "depth":
                ...
                if corr_radius == -1:  # global matching
                    if task == "flow":
                        ...
                        flow_pred = global_correlation_softmax_stereo(
                            feature0, feature1
                        )[0]
                    else:
                        raise NotImplementedError
                    ...
                    if task == "flow":
                        ...
                        flow_pred = local_correlation_softmax_stereo(
                            feature0, feature1, corr_radius
                        )[0]
                    else:
                        raise NotImplementedError
            ...
            if scale_idx == self.num_scales - 1:
                if not self.reg_refine:
                    ...
                    if task == "stereo":
                        flow_pad = torch.cat(
                            (-flow, torch.zeros_like(flow)), dim=1
                        )  # [B, 2, H, W]
                        ...
                        depth_up_pad = self.upsample_flow(
                            depth_pad, feature0, is_depth=True
                    ...
                    for refine_iter_idx in range(num_reg_refine):
                        ...
                        if task == "stereo":
                            ...
                            if pred_bidir_depth and refine_iter_idx == 0:
                                ...
                                feature0_ori, feature1_ori = torch.cat(
                                    (feature0_ori, feature1_ori), dim=0
