...
class Logger:
    ...
    def print_training_status(self, mode="train", is_depth=False):
        if is_depth:
            print(
                ...
                % (
                    self.total_steps,
                    self.running_loss["total_loss"] / self.summary_freq,
                )
            )
        else:
            print(
        ...
        for k in self.running_loss:
            self.summary_writer.add_scalar(
                mode + "/" + k,
                self.running_loss[k] / self.summary_freq,
                self.total_steps,
            )
        ...
        if self.total_steps % self.summary_freq == 0:
            if is_depth:
                ...
                img_concat = img_concat.type(
                    torch.uint8
                )  # convert to uint8 to visualize in tensorboard

                flow_pred = flow_tensor_to_image(flow_preds[-1][0])
                forward_flow_gt = flow_tensor_to_image(flow_gt[0])
                flow_concat = torch.cat(
                    (torch.from_numpy(flow_pred), torch.from_numpy(forward_flow_gt)),
                    dim=-1,
                )
                ...
                self.summary_writer.add_image(
                    mode + "/img_pred_gt", concat, self.total_steps
                )
    ...
    def add_depth_summary(self, depth_pred, depth_gt, mode="train"):
        ...
        if self.total_steps % self.summary_freq == 0 or "val" in mode:
            ...
            self.summary_writer.add_image(
                mode + "/depth_pred_gt", concat, self.total_steps
            )
