...
from src.transformer.anima.position_embedding import VideoRopePosition3DEmb, LearnablePosEmbAxis
...
def torch_attention_op(q_B_S_H_D: torch.Tensor, k_B_S_H_D: torch.Tensor, v_B_S_H_D: torch.Tensor, transformer_options: Optional[dict] = {}) -> torch.Tensor:
    ...
    attn_out = attention_register.call(q_B_H_S_D, k_B_H_S_D, v_B_H_S_D)
    return rearrange(attn_out, "b h s d -> b s (h d)", b=b, s=s, h=h, d=d)
...
class AnimaTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin):
    """
    ...
    def forward(
        ...
        fps: Optional[torch.Tensor] = None,
        padding_mask: Optional[torch.Tensor] = None,
        # Some engines (including our Anima engine) use the diffusers-style kwarg name `timestep`.
        ...
        **kwargs,
        ...
        if timesteps is None:
            timesteps = timestep
        if timesteps is None:
            raise ValueError("`timesteps` (or alias `timestep`) must be provided.")
        if context is None:
