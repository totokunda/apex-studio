...
class HunyuanVideoFoley(ModelMixin, ConfigMixin):
    ...
    def __init__(
        self,
        model_name: str = "hunyuanvideo-foley",
        model_type: str = "1d",
        ...
        self.patch_size = 1
        self.audio_embedder = PatchEmbed1D(self.patch_size, self.audio_vae_latent_dim, self.hidden_size)
        ...
        if self.condition_projection == "linear":
            self.cond_in = ConditionProjection(
                self.condition_dim, self.hidden_size, get_activation_layer("silu")
            )
        else:
        ...
        self.final_layer = FinalLayer1D(
            self.hidden_size, self.patch_size, self.out_channels, get_activation_layer("silu")
        )
        self.unpatchify_channels = self.out_channels
