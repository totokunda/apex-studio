...
def _chunked_modulated_norm(
    ...
    for i in range(0, s, chunk_size):
        ...
        if scale_per_token:
            ...
            scale_chunk = scale
            shift_chunk = shift
        out[:, i:end, :] = norm_layer(hs_chunk) * (1 + scale_chunk) + shift_chunk
    return out


def _chunked_norm(norm_layer: nn.Module, hidden_states: torch.Tensor, chunk_size: Optional[int] = 8192) -> torch.Tensor:
    """
...
def _chunked_feed_forward(ff: nn.Module, hidden_states: torch.Tensor, chunk_dim: int, chunk_size: Optional[int]):
    """
...
def _chunked_apply_scale_shift(
    hidden_states: torch.Tensor, scale: torch.Tensor, shift: torch.Tensor, chunk_size: Optional[int]
) -> torch.Tensor:
    """
    ...
    for i in range(0, s, chunk_size):
        ...
        if scale_per_token:
            ...
            scale_chunk = scale
            shift_chunk = shift
        out[:, i:end, :] = hs_chunk * (1 + scale_chunk) + shift_chunk
    return out
...
class LTX2VideoTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self._mod_norm_chunk_size: Optional[int] = None  # for norms followed by modulation
    ...
    def forward(
        ...
        norm_audio_hidden_states = _chunked_norm(self.audio_norm2, audio_hidden_states, chunk_size=self._norm_chunk_size)
        ...
        norm_audio_hidden_states = _chunked_norm(
            self.video_to_audio_norm, audio_hidden_states, chunk_size=self._norm_chunk_size
        )
        ...
        audio_ff_output = _chunked_feed_forward(
            self.audio_ff, norm_audio_hidden_states, self._ff_chunk_dim, self._ff_chunk_size
        )
