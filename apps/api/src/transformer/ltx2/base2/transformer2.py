...
# VRAM Optimizations by DeepBeepMeep (c) 2026.

from dataclasses import dataclass, replace

import torch

from src.engine.ltx22.shared.guidance.perturbations import BatchedPerturbationConfig, PerturbationType
from src.transformer.ltx2.base2.adaln import adaln_embedding_coefficient
from src.transformer.ltx2.base2.attention import Attention, AttentionCallable
from src.transformer.ltx2.base2.feed_forward import FeedForward
from src.transformer.ltx2.base2.rope import LTXRopeType
from src.transformer.ltx2.base2.transformer_args import TransformerArgs
from src.engine.ltx22.shared.utils import rms_norm
...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def _apply_text_cross_attention(
        ...
        input_list = [rms_norm(x, eps=self.norm_eps)]
        context_list = [context]
        del x, context
        return attn(input_list, context_list=context_list, mask=context_mask)
    ...
    def forward(  # noqa: PLR0915
        ...
        if run_vx:
            ...
            norm_vx = _apply_scale_shift(rms_norm(vx, eps=self.norm_eps), vscale_msa, vshift_msa)
        ...
        if run_ax:
            ...
            norm_ax = _apply_scale_shift(rms_norm(ax, eps=self.norm_eps), ascale_msa, ashift_msa)
        ...
        if run_a2v or run_v2a:
            ...
            if run_a2v and not perturbations.all_in_batch(PerturbationType.SKIP_A2V_CROSS_ATTN, self.idx):
                ...
                vx_scaled = _apply_scale_shift(vx_norm3, scale_ca_video_a2v, shift_ca_video_a2v)
                ...
                ax_scaled = _apply_scale_shift(ax_norm3, scale_ca_audio_a2v, shift_ca_audio_a2v)
                del scale_ca_audio_a2v, shift_ca_audio_a2v
                a2v_mask = perturbations.mask_like(PerturbationType.SKIP_A2V_CROSS_ATTN, self.idx, vx)
                vx_list = [vx_scaled]
                context_list = [ax_scaled]
                del vx_scaled, ax_scaled
                vx.add_(
                    _apply_gate(self.audio_to_video_attn(
                        vx_list,
                        context_list,
                        pe=video.cross_positional_embeddings,
                        k_pe=audio.cross_positional_embeddings,
                    ), gate_out_a2v).mul_(a2v_mask)
                    )
            ...
            if run_v2a and not perturbations.all_in_batch(PerturbationType.SKIP_V2A_CROSS_ATTN, self.idx):
                ...
                ax_scaled = _apply_scale_shift(ax_norm3, scale_ca_audio_v2a, shift_ca_audio_v2a)
                ...
                vx_scaled = _apply_scale_shift(vx_norm3, scale_ca_video_v2a, shift_ca_video_v2a)
                del scale_ca_video_v2a, shift_ca_video_v2a
                v2a_mask = perturbations.mask_like(PerturbationType.SKIP_V2A_CROSS_ATTN, self.idx, ax)
                ax.add_(
                    _apply_gate(self.video_to_audio_attn(
                        ax_scaled,
                        context=vx_scaled,
                        pe=audio.cross_positional_embeddings,
                        k_pe=video.cross_positional_embeddings,
                    ), gate_out_v2a).mul_(v2a_mask)
                )
        ...
        if run_vx:
            ...
            vx_scaled = _apply_scale_shift(rms_norm(vx, eps=self.norm_eps), vscale_mlp, vshift_mlp)
            vx.add_(
                _apply_gate(self.ff(vx_scaled), vgate_mlp)
            )
        ...
        if run_ax:
            ...
            ax_scaled = _apply_scale_shift(rms_norm(ax, eps=self.norm_eps), ascale_mlp, ashift_mlp)
            ax.add_(
                _apply_gate(self.audio_ff(ax_scaled), agate_mlp)
            )
...
def apply_cross_attention_adaln(
    ...
    attn_input = _apply_scale_shift(rms_norm(x, eps=norm_eps), q_scale, q_shift)
    encoder_hidden_states = _apply_scale_shift(context, scale_kv, shift_kv)
    input_list = [attn_input]
    context_list = [encoder_hidden_states]
    del attn_input, encoder_hidden_states
    return attn(input_list, context_list=context_list, mask=context_mask).mul_(q_gate)
