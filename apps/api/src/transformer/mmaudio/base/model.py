...
from src.transformer.mmaudio.base.embeddings import TimestepEmbedder
from src.transformer.mmaudio.base.low_level import MLP, ChannelLastConv1d, ConvMLP
from src.transformer.mmaudio.base.transformer_layers import (FinalBlock, JointBlock, MMDitSingleBlock)
from src.transformer.mmaudio.base.rotary_embeddings import compute_rope_rotations
...
class MMAudioTransformer(ModelMixin, ConfigMixin, FromOriginalModelMixin, PeftAdapterMixin):
    # These init args are tensors (not JSON-serializable) and will later be registered as
    # buffers/parameters. If they end up in `self._internal_dict`, `ModelMixin.__getattr__`
    # will make `hasattr(self, ...)` return True and PyTorch will refuse to register a
    # parameter/buffer with the same name ("attribute already exists").
    ignore_for_config = ["latent_mean", "latent_std", "empty_string_feat"]
    ...
    def __init__(self,
        ...
        # Non-trainable tensors that should be part of the state dict.
        self.latent_mean = nn.Buffer(latent_mean.view(1, 1, -1))
        self.latent_std = nn.Buffer(latent_std.view(1, 1, -1))
        self.empty_string_feat = nn.Buffer(empty_string_feat)
    ...
    def forward(self, latent: torch.Tensor, t: torch.Tensor, conditions: PreprocessedConditions, return_dict: bool = False) -> torch.Tensor:
        """
        latent: (B, N, C) 
        t: (B,)
...
def small_16k(**kwargs) -> MMAudioTransformer:
    num_heads = 7
    return MMAudioTransformer(latent_dim=20,
                   clip_dim=1024,
                   sync_dim=768,
                   text_dim=1024,
...
def small_44k(**kwargs) -> MMAudioTransformer:
    num_heads = 7
    return MMAudioTransformer(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
                   text_dim=1024,
                   hidden_dim=64 * num_heads,
...
def medium_44k(**kwargs) -> MMAudioTransformer:
    num_heads = 14
    return MMAudioTransformer(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
...
def large_44k(**kwargs) -> MMAudioTransformer:
    num_heads = 14
    return MMAudioTransformer(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
...
def large_44k_v2(**kwargs) -> MMAudioTransformer:
    num_heads = 14
    return MMAudioTransformer(latent_dim=40,
                   clip_dim=1024,
                   sync_dim=768,
