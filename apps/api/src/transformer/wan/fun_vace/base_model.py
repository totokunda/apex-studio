...
def cfg_skip():
    def decorator(func):
        def wrapper(self, hidden_states, *args, **kwargs):
            ...
            if (
                bs >= 2
                and self.cfg_skip_ratio is not None
                and self.current_steps
                >= self.num_inference_steps * (1 - self.cfg_skip_ratio)
            ):
                bs_half = int(bs // 2)
...
class TeaCache:
    ...
    def __init__(
        ...
        if rel_l1_thresh < 0:
            raise ValueError(
                f"`rel_l1_thresh` must be greater than or equal to 0 but is {rel_l1_thresh}."
            )
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        temp_x = _chunked_modulated_norm(
...
class WanTransformer3DModel(
    ...
    def __init__(
        ...
        if add_ref_conv:
            self.ref_conv = nn.Conv2d(
                in_dim_ref_conv, dim, kernel_size=patch_size[1:], stride=patch_size[1:]
            )
        else:
            self.ref_conv = None
    ...
    def set_chunking_profile(self, profile_name: str) -> None:
        ...
        self.head.set_chunk_norms(
            modulated_norm_chunk_size=p.get("head_modulated_norm_chunk_size", None)
        )

    def _set_gradient_checkpointing(self, *args, **kwargs):
        if "value" in kwargs:
            self.gradient_checkpointing = kwargs["value"]
            if hasattr(self, "motioner") and hasattr(
                self.motioner, "gradient_checkpointing"
            ):
    ...
    def forward(
        ...
        if self.ref_conv is not None and full_ref is not None:
            ...
            grid_sizes = torch.stack(
                [torch.tensor([u[0] + 1, u[1], u[2]]) for u in grid_sizes]
            ).to(grid_sizes.device)
            seq_len += full_ref.size(1)
            x = [
                torch.concat([_full_ref.unsqueeze(0), u], dim=1)
                for _full_ref, u in zip(full_ref, x)
            ]
        ...
        if subject_ref is not None:
            ...
            x = [
                torch.concat([u, _subject_ref.unsqueeze(0)], dim=1)
                for _subject_ref, u in zip(subject_ref, x)
            ]
            if t.dim() != 1 and t.size(1) < seq_len:
                pad_size = seq_len - t.size(1)
                last_elements = t[:, -1].unsqueeze(1)
            ...
            try:
                ...
                if (
                    ...
                    model.state_dict()["patch_embedding.weight"][
                        ...
                        :,
                        : model.state_dict()["patch_embedding.weight"].size()[1],
                        :,
                        :,
                    ]
                    model.state_dict()["patch_embedding.weight"][
