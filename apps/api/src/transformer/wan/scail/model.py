...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens):
        ...
        # compute attention
        x = attention_register.call(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), k_lens=context_lens, window_size=self.window_size).transpose(1, 2)
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens):
        ...
        img_x = attention_register.call(q.transpose(1, 2), k_img.transpose(1, 2), v_img.transpose(1, 2), k_lens=None, window_size=self.window_size).transpose(1, 2)
        # compute attention
        x = attention_register.call(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), k_lens=context_lens, window_size=self.window_size).transpose(1, 2)
