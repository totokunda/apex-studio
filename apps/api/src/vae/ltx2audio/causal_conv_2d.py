...
class CausalConv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.causality_axis.value == "none":
            self.padding = (pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2)
        elif self.causality_axis.value == "width" or self.causality_axis.value == "width-compatibility":
            self.padding = (pad_w, 0, pad_h // 2, pad_h - pad_h // 2)
        elif self.causality_axis.value == "height":
            self.padding = (pad_w // 2, pad_w - pad_w // 2, pad_h, 0)
        else:
            raise ValueError(f"Invalid causality_axis: {self.causality_axis.value}")
