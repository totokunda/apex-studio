...
def clear_memory(
    ...
    if not force:
        ...
        if "error" not in mem_info and mem_info["total_gb"] > 0:
            ...
            if free_ratio < 0.05:
                ...
                if debug:
                    ...
                    debug.log(
                        f"{backend} pressure: {mem_info['free_gb']:.2f}GB free of {mem_info['total_gb']:.2f}GB",
                        category="memory",
                    )
        ...
        if not should_clear and not torch.backends.mps.is_available():
            ...
            if mem.available < mem.total * 0.05:
                ...
                if debug:
                    debug.log(
                        f"RAM pressure: {mem.available/(1024**3):.2f}GB free of {mem.total/(1024**3):.2f}GB",
                        category="memory",
                    )
    ...
    if deep:
        ...
        try:
            ...
            if debug:
                debug.log(
                    f"Failed to perform OS memory operations: {e}",
                    level="WARNING",
                    category="memory",
                    force=True,
                )
...
def retry_on_oom(func, *args, debug=None, operation_name="operation", **kwargs):
    ...
    try:
        ...
        if not any(
            x in str(e).lower() for x in ["out of memory", "allocation on device"]
        ):
            raise

        if debug:
            debug.log(
                f"OOM during {operation_name}: {e}",
                level="WARNING",
                category="memory",
                force=True,
            )
        ...
        time.sleep(0.5)
        if debug:
            debug.log_memory_state(
                "After memory clearing", show_tensors=False, detailed_tensors=False
            )
        ...
        try:
            ...
            if debug:
                debug.log(
                    f"Retry successful for {operation_name}",
                    category="success",
                    force=True,
                )
            return result
