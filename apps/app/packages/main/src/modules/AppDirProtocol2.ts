...
class AppDirProtocol implements AppModule {
  ...
  async enable({ app }: ModuleContext): Promise<void> {
    ...
    protocol.handle("app", async (request) => {
      ...
      if (u.hostname === "apex-cache") {
        ...
        // Before probing remote, try to resolve a local cache hit from known local bases.
        const decodedPathname = this.safeDecodeURIComponent(u.pathname);
        const relCandidates = this.relCandidatesForLookup(decodedPathname);
        if (relCandidates.length > 0 && this.electronApp) {
          const localHit = await this.findExistingLocalCacheFile(
            relCandidates,
            this.electronApp,
          );
          if (localHit) {
            ...
            return await this.serveLocalFile(request, localHit.filePath, baseCors);
          }
        }

        // If we're not in remote-mode yet, try probing and switching.
        if (!this.loopbackAppearsRemote) {
          const candidates = this.relCandidatesForLookup(
            this.safeDecodeURIComponent(u.pathname),
          );
  ...
  private relCandidatesForLookup(decodedPathname: string): string[] {
    const normalized = (decodedPathname || "").replace(/\\/g, "/");
    const candidates: string[] = [];
    try {
      const rel = this.remoteRelFromNormalized(normalized);
      if (rel) candidates.push(rel);
    } catch {
      // ignore
    }
    candidates.push(...this.remoteRelCandidatesFromPath(normalized));
    return Array.from(
      new Set(candidates.map((c) => this.stripLeadingSlashes(c)).filter(Boolean)),
    );
  }

  private localCacheBaseCandidates(app: Electron.App): string[] {
    const bases: string[] = [];
    if (this.cachePath) bases.push(this.cachePath);
    try {
      bases.push(path.join(app.getPath("userData"), "apex-cache"));
      const home = os.homedir();
      if (home) bases.push(path.join(home, "apex-diffusion", "cache"));
      const settingsPath = path.join(app.getPath("userData"), "apex-settings.json");
      if (fs.existsSync(settingsPath)) {
        const raw = fs.readFileSync(settingsPath, "utf-8");
        const parsed = JSON.parse(raw);
        if (parsed.cachePath && typeof parsed.cachePath === "string") {
          bases.push(path.resolve(parsed.cachePath));
        }
      }
    } catch {
      // ignore
    }
    return Array.from(new Set(bases.map((b) => path.resolve(b))));
  }

  private async findExistingLocalCacheFile(
    relCandidates: string[],
    app: Electron.App,
  ): Promise<{ filePath: string; relPath: string } | null> {
    const bases = this.localCacheBaseCandidates(app);
    if (bases.length === 0) return null;
    for (const rel of relCandidates) {
      const relClean = this.stripLeadingSlashes(rel);
      if (!relClean) continue;
      for (const base of bases) {
        const candidate = this.resolveUnderBase(base, relClean);
        if (!candidate) continue;
        try {
          const st = await fs.promises.stat(candidate);
          if (st.isFile() && st.size > 0) {
            return { filePath: candidate, relPath: relClean };
          }
        } catch {
          // ignore
        }
