...
export function createMergeAlphaState(): MergeAlphaState {
    return {
        mergeGlCanvas: null,
        mergeGl: null,
        mergeGlProgram: null,
...
function ensureMergeGl(state: MergeAlphaState, width: number, height: number): boolean {
...
function ensureMergeCanvases(state: MergeAlphaState, width: number, height: number) {
    if (
      !state.mergeCanvas ||
