...
function applyTextTransform(text: string, textTransform?: TextClipProps['textTransform']): string {
...
function withStage<T>(canvas: Canvas, draw: (layer: Layer) => T): T {
  ...
  try {
    ...
    if (ctx) {
      ...
      const sceneCanvas = stage.toCanvas({ pixelRatio: 1 }) as unknown as HTMLCanvasElement;
...
export async function blitDrawing(
  ...
  const run = async () => {
    ...
    const stage = new Stage({ container, width: canvas.width, height: canvas.height });
    const layer = new Layer();
    stage.add(layer);
    ...
    try {
      ...
      for (const l of lines) {
        ...
        let lineCanvas = stage.toCanvas({ pixelRatio: 1 }) as unknown as HTMLCanvasElement;
...
export function resolveVideoSourceForFrame(c: VideoClipProps, projectFrame: number): { selectedSrc: string; frameOffset: number } {
