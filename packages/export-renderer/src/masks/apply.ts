...
export function applyMasksToCanvas(
  ...
  let frame = typeof maskFrameOverride === 'number' ? Math.max(0, Math.floor(maskFrameOverride)) : focusFrame;
  if (clip && typeof maskFrameOverride !== 'number') {
    frame = clip.type === 'video' ? Math.max(0, Math.round(getLocalFrame(focusFrame, clip))) : 0;
  }
  ...
  try {
    ...
    for (let i = 0; i < masks.length; i++) {
      ...
      const effectiveMask: MaskClipProps = i === 0 ? mask : { ...mask, backgroundColorEnabled: false };
      ...
      if (effectiveMask.tool === 'shape') {
        maskedCanvas = shapeMask.apply(acc, effectiveMask as any, frame, clip?.transform as any, baseTransform as any, debug);
      } else if (effectiveMask.tool === 'lasso') {
        maskedCanvas = lassoMask.apply(acc, effectiveMask as any, frame, clip?.transform as any, baseTransform as any, debug);
      } else if (effectiveMask.tool === 'touch') {
        maskedCanvas = touchMask.apply(acc, effectiveMask as any, frame, clip?.transform as any, baseTransform as any, debug);
      }
    ...
    try { shapeMask.dispose(); } catch {}
    try { lassoMask.dispose(); } catch {}
    try { touchMask.dispose(); } catch {}
  }
}
