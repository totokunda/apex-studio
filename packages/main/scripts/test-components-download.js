...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === '--paths' && next) { args.paths = next; i++; continue; }
  }
  return args;
}
...
async function main() {
  ...
  ws.on('message', (buf) => {
    try {
      ...
      const pct = typeof msg.progress === 'number' ? (msg.progress * 100).toFixed(1) : 'n/a';
      const label = msg?.metadata?.label ? ` [${msg.metadata.label}]` : '';
      console.log(`progress=${pct}% status=${msg.status || 'processing'}${label} ${msg.message || ''}`);
    } catch (e) {
  ...
  while (!done) {
    ...
    const s = await fetch(`${url}/components/status/${jobId}`).then((r) => r.json()).catch(() => ({}));
    if (s && (s.status === 'complete' || s.status === 'error' || s.status === 'cancelled')) {
