...
const findPngFiles = async (dir: string, baseDir: string): Promise<string[]> => {
    const files: string[] = [];
...
export const fetchFilters = async () => {
    ...
    const publicPath = join(__dirname, '..', '..', '..', 'packages', 'renderer', 'public', 'filters');
    ...
    for (const relPath of smallFiles) {
        ...
        filters.push({ id, name, smallPath, fullPath, category, examplePath, exampleAssetUrl });
    }
