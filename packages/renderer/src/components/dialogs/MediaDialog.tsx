...
export const MediaDialog: React.FC<MediaDialogProps> = ({
    ...
    const {getClipTransform, clips, timelines, clipWithinFrame} = useClipStore();
    ...
    const isPlaying = typeof isPlayingExternal === "boolean" ? isPlayingExternal : useControlsStore((state) => state.isPlaying);
    ...
    const handleScrubberMove = useCallback((clientX: number) => {
        if (!progressBarRef.current || !selectedClip || selectedClip.type !== 'video') return;
    ...
    const isSelectionPlaying = typeof isPlayingExternal === "boolean" ? isPlayingExternal : isPlaying;
    ...
    const handleConfirm = () => {
        ...
        const { width: dispW, height: dispH, x: dispX, y: dispY } = mediaDisplayRect;
    ...
    const toRender = useMemo(() => {
      ...
      }, [previewRect.width, previewRect.height, toRender, clipOverride, selectedClip]);
    ...
    useEffect(() => {
        ...
        if (selectedClip.transform?.crop) {
             ...
             const { x: nX, y: nY, width: nW, height: nH } = selectedClip.transform.crop;
    ...
    }, [mediaDisplayRect, selectedClip?.transform, selectedClip?.clipId, getOverlayPath]); // Re-run explicitly when clipId changes
        ...
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            <DialogContent className="max-w-4xl w-full h-[80vh] flex flex-col bg-background text-foreground p-0 gap-0 overflow-hidden dark font-poppins border-brand-light/10">
                <DialogHeader className="px-6 py-6 border-b border-brand-light/10 flex-shrink-0 bg-brand-background">
                    <DialogTitle className="text-start text-[13px] font-medium"> </DialogTitle>
                </DialogHeader>
                <div 
                ...
                >
                   {!isValidClip ? (
                   ...
                   ) : toRender.every(clip => clip.type === 'audio') && timelineSelectorProps ? (
                    <>
                    <AudioPreview
                    ...
                    />
                    <CircularAudioVisualizer inputId={timelineSelectorProps.inputId} width={size.width} height={size.height} active={isSelectionPlaying} />
                  </>
                   ) : (
                       <div className="relative w-full h-full flex flex-col">
                            <div className="flex-1 relative flex items-center justify-center overflow-hidden">
                                <Stage width={size.width + 32} height={size.height + 64} className="bg-transparent">
                                    <Layer>
                                        <Group
                                            x={view.x}
                                            y={view.y}
                            ...
                            {isValidClip && toRender.some(clip => clip.type === 'video') && !timelineSelectorProps && (
                ...
                {timelineSelectorProps && (
                    <div ref={divTimelineSelectorRef} className="px-6 py-4 border-t border-brand-light/10 bg-brand-background flex-shrink-0 flex items-center justify-between">
                        <TimelineSelector clip={clipOverride ?? selectedClip as AnyClipProps} height={64} width={(divTimelineSelectorRef.current?.clientWidth ?? 0) - 60} mode={timelineSelectorProps.mode} inputId={timelineSelectorProps.inputId} />
                    </div>
                )}
                <div className="px-6 py-4 border-t border-brand-light/10 bg-brand-background flex-shrink-0 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6 flex-1">
                         ...
                         {canCrop && <div className="flex items-center gap-3">
                            ...
                            >
                                ...
                                <SelectContent className="bg-background text-foreground font-poppins dark">
                                    {ASPECT_RATIOS.map((ratio) => (
                                        <SelectItem key={ratio.value} value={ratio.value} className="text-brand-light text-[11px] font-medium">
                                            {ratio.label}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
