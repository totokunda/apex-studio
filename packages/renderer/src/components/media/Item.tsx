    ...
    return (
      <AlertDialog open={open} onOpenChange={onOpenChange}>
        <AlertDialogContent className='dark bg-brand font-poppins'>
          <AlertDialogHeader>
            <AlertDialogTitle className='text-brand-light text-base'>Delete Media</AlertDialogTitle>
            <AlertDialogDescription className='text-brand-light/70 py-0 text-sm'>Are you sure you want to delete this media? This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => onOpenChange(false)} className='bg-brand text-brand-light cursor-pointer text-[12.5px]'>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => {onDelete(); onOpenChange(false)}} className='cursor-pointer bg-brand-lighter text-[12.5px]'>Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    )
...
export const MediaThumb: React.FC<{ item: MediaItem, isDragging?: boolean }> = React.memo(({ item, isDragging }) => {
    ...
    useEffect(() => {
      ...
      (async () => {
        try {
          ...
          const waveform = await generateAudioWaveformCanvas(item.assetUrl, el.width, el.height, { color: '#7791C4', mediaInfo: item.mediaInfo });
          if (item.mediaInfo) {
    ...
    useEffect(() => {
      ...
      (async () => {
        try {
          const poster = await generatePosterCanvas(item.assetUrl, undefined, undefined, { mediaInfo: item.mediaInfo });
          if (item.mediaInfo) {
            setMediaInfo(item.mediaInfo);
          }
      ...
      <Draggable id={item.name} data={item} disabled={isOpen || renamingItem === item.name || deleteAlertOpen || isCreatingProxy}>
              <div key={item.name} className={cn("group rounded-md transition-colors p-2.5  hover:bg-brand-light/5 ", {
                ...
                {renamingItem === item.name ? (
                  ...
                  <div className="px-0 pt-2 text-[10.5px] cursor-pointer text-brand-light/90 text-start truncate">{item.name}</div>
                )}
              </div>
              <DeleteAlertDialog open={deleteAlertOpen} onOpenChange={setDeleteAlertOpen} onDelete={() => deleteItem && handleDelete(deleteItem)} />   
        </Draggable>
  )
}
