...
const LoRACard: React.FC<{ item: LoraType; manifestId: string }> = ({
  item,
  manifestId,
}) => {
  ...
  const scale = useMemo(
    () => (typeof item !== "string" ? item?.scale : undefined),
    [item],
  );
    ...
    async (completed: string | string[]) => {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("lora-card-reload", {
            detail: { paths: list, manifestId },
          }),
        );
      } catch {}
      runRef.current?.();
...
const DownloadedLoRASection: React.FC<DownloadedLoRASectionProps> = ({
  deleting,
  handleDelete,
}) => {
  return (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  {f.status === "completed" || f.status === "complete" ? (
                    ...
                    <div className="text-[9px] text-brand-light/50">
...
const IdleDownloadSection: React.FC<IdleDownloadSectionProps> = ({
  downloadingPaths,
  handleDownload,
  path,
}) => {
  return (
    ...
    >
      ...
      <span>
        {downloadingPaths.has(path || "") ? "Downloading..." : "Download LoRA"}
      </span>
    </button>
  );
};
