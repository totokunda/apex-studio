...
const LoRACard: React.FC<{ item: LoraType; manifestId: string }> = ({ item, manifestId }) => {
  ...
  const isActive = !!path && (downloadingPaths.has(path) || files.some((f) => f?.status === 'processing' || f?.status === 'pending'));
  ...
  const onDownloadComplete = useCallback(async (completed: string | string[]) => {
  ...
  useEffect(() => {
    ...
    return () => {
      try { window.removeEventListener('lora-card-reload', handler as EventListener); } catch {}
    };
  }, [manifestId, path]);
        ...
        <div className="px-4 pb-4">
          <div className="space-y-2 mt-1">
            ...
            <div className="flex gap-x-1.5 text-[10px]">
                <span className="text-brand-light/70 font-medium block">Weight Scale</span> <span className="text-brand-light font-mono block">{scale.toFixed(2)}</span>
              </div>
            </div>
          )}
                          ...
                          <div key={f.filename || f.label || idx} className="flex flex-col gap-y-1">
                            <div className="flex items-center justify-between gap-x-2 w-full">
                              <div className="flex-1 min-w-0">
                                <div className="text-[10px] text-brand-light/80 font-mono truncate break-all">{f.filename || f.label || path}</div>
                              </div>
