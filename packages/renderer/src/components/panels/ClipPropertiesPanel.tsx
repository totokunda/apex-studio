...
const ClipPropertiesPanel: React.FC<PropertiesPanelProps> = ({ panelSize }) => {
  ...
  const preprocessor = selectedPreprocessorId
    ? getPreprocessorById(selectedPreprocessorId)
    : null;
  ...
  const [selectedTab, setSelectedTab] = useState<string>(
    clip?.type === "text" ? "text" : "transform",
  );
  const { clearJob, stopTracking } = usePreprocessorJobActions();
  const {
    startTracking: startEngineTracking,
    stopTracking: stopEngineTracking,
    clearJob: clearEngineJob,
  } = useEngineJobActions();
  ...
  const { preprocessors, load: loadPreprocessors } =
  ...
  const currentPreprocessors =
    getPreprocessorsForClip(clip?.clipId ?? "") || [];
  ...
  const handleAddPreprocessor = useCallback(
    (preproc: Preprocessor) => {
      ...
      const fullyCovered = (() => {
        ...
        const intervals = existingAll
          .map((p) => {
            const s = Math.max(0, p.startFrame ?? 0);
            const e = Math.max(
              s + 1,
              Math.min(clipDuration, p.endFrame ?? clipDuration),
            );
      ...
      if (fullyCovered) {
        try {
          toast.info("No available space for another preprocessor");
        } catch {}
        return;
      }
      ...
      try {
        setSelectedPreprocessorId(id);
      } catch {}
      try {
        toast.success(`Added ${preproc.name}`);
      } catch {}
    },
    [
      clip,
  ...
  const {
    ...
    isComplete: isEngineComplete,
    isFailed: isEngineFailed,
  } = useEngineJob(effectiveJobId, false);
  ...
  const buildSelectedComponentDefaults = useCallback(
    (manifest: any): Record<string, any> => {
  ...
  useEffect(() => {
    ...
    const existing =
      (clip as ModelClipProps | undefined)?.selectedComponents || {};
    const merged = { ...defaults, ...existing };
    if (!_.isEqual(merged, existing)) {
      try {
        updateClip(clipId, { selectedComponents: merged } as any);
      } catch {}
    }
  }, [
    hasModel,
    clipId,
    manifestData,
    buildSelectedComponentDefaults,
    updateClip,
    clip,
  ]);
  ...
  const isModelDownloaded = useMemo(() => {
    ...
    return components.every((comp) => {
      ...
      if (typeof (comp as any).is_downloaded === "boolean")
        return !!(comp as any).is_downloaded;
      return true;
    });
  ...
  const isGenerateDisabled = useMemo(() => {
    ...
    const isEmpty = (val: any) => {
      ...
      if (
        val &&
        typeof val === "object" &&
        Object.prototype.hasOwnProperty.call(val, "selection")
      ) {
    ...
    return ui.inputs.some(
      (inp: any) => inp?.required && isEmpty(values[inp.id]),
    );
  ...
  const getValidTab = (currentTab: string) => {
    ...
    if (currentTab === "preprocessors" && hasPreprocessorBrowser)
      return "preprocessors";
    if (currentTab === "audio" && hasAudio) return "audio";
    if (currentTab === "duration" && (hasDuration || hasFilter))
      return "duration";
    ...
    if (currentTab === "preprocessor-parameters" && hasValidPreprocessor)
      return "preprocessor-parameters";
    if (
      currentTab === "preprocessor-duration" &&
      hasValidPreprocessor &&
      hasPreprocessorDuration
    )
      return "preprocessor-duration";
    if (currentTab === "model-inputs" && hasModel) return "model-inputs";
    if (
      currentTab === "model-progress" &&
      hasModel &&
  ...
  useEffect(() => {
    ...
    hasPreprocessorDuration,
    tool,
    (clip as ModelClipProps | undefined)?.modelStatus,
    hasFrameInterpolate,
  ]);
  ...
  const handleStopGeneration = useCallback(async () => {
    ...
    try {
      await stopEngineTracking(targetJobId);
    } catch {}
    try {
      clearEngineJob(targetJobId);
    } catch {}
    try {
      if (clipId) updateClip(clipId, { modelStatus: undefined });
    } catch {}
  }, [
    clipId,
    engineJobId,
    stopEngineTracking,
    clearEngineJob,
    (clip as ModelClipProps)?.activeJobId,
  ]);
  ...
  return (
    ...
    >
        ...
        >
          <div className="relative flex-shrink-0 ">
            <LuChevronLeft
              ...
              className={cn(
                "text-brand-light h-6 w-6 bg-brand-background-dark/90  backdrop-blur-sm border border-brand-light/10 hover:bg-brand-background/100 z-50 transition-all duration-200 rounded-full absolute left-0 top-1/2 -translate-y-1/2 p-1 cursor-pointer ",
            ...
            <TabsList
              ...
              className={cn(
                "bg-brand w-full rounded-b-none p-0 min-w-0 flex-shrink overflow-x-auto [&::-webkit-scrollbar]:hidden",
              )}
            >
              {hasValidPreprocessor && (
                <TabsTrigger
                  value="preprocessor-parameters"
                  className="text-brand-light text-[11px] h-9 flex-shrink-0 px-4.5 whitespace-nowrap"
                >
                  Inputs
                </TabsTrigger>
              )}
              {hasValidPreprocessor && hasPreprocessorDuration && (
                <TabsTrigger
                  value="preprocessor-duration"
                  className="text-brand-light text-[11px] h-9 flex-shrink-0 px-4.5 whitespace-nowrap"
                >
                  Duration
                </TabsTrigger>
              ...
              {hasPreprocessorBrowser && !hasMask && (
                <TabsTrigger
                  value="preprocessors"
                  className="text-brand-light text-[11px] h-9 flex-shrink-0 px-4.5 whitespace-nowrap"
                >
            ...
            <LuChevronRight
              ...
              className={cn(
                "text-brand-light h-6 w-6 border border-brand-light/10 bg-brand-background-dark/90 backdrop-blur-sm hover:bg-brand-background/100 z-10 transition-all duration-200 rounded-full absolute right-0 top-1/2 -translate-y-1/2 p-1 cursor-pointer ",
          ...
          <ScrollArea className="flex-1 overflow-y-auto">
            <div style={{ paddingBottom: hasValidPreprocessor ? "20px" : "0" }}>
              {hasPreprocessorBrowser && (
                <TabsContent value="preprocessors" className="min-w-0 m-0">
              ...
              {hasValidPreprocessor &&
                ...
                hasPreprocessorDuration && (
                  <TabsContent
                    ...
                    className="min-w-0 m-0"
                  >
                    <PreprocessorDurationPanel
                      preprocessorId={selectedPreprocessorId}
                    />
                  </TabsContent>
                )}
              {hasLine && (
                <TabsContent value="line" className="min-w-0 m-0">
                  <LineProperties clipId={clipId} />
                </TabsContent>
              )}
              {hasText && (
                <TabsContent value="text" className="min-w-0 m-0">
                  {" "}
                  <TextProperties clipId={clipId} />{" "}
                </TabsContent>
              )}
              {hasTransform && (
                <TabsContent
                  value="transform"
                  className="min-w-0 divide-y divide-brand-light/10 m-0"
                >
              ...
              {hasMask && (
                <TabsContent value="mask" className="min-w-0 m-0">
                  <MaskPropertiesPanel clipId={clipId} />
                </TabsContent>
              )}
              {hasAudio && (
                <TabsContent value="audio" className="min-w-0 m-0">
              ...
              {hasAdjust && (
                <TabsContent value="adjust" className="min-w-0 m-0">
                  <AdjustProperties clipId={clipId} />
                </TabsContent>
              )}
              {hasAppearance && (
                <TabsContent value="appearance" className="min-w-0 m-0">
                  <AppearanceProperties clipId={clipId} />
                </TabsContent>
              )}
              {hasModel && (
                <TabsContent value="model-inputs" className="min-w-0 m-0">
                  <ModelInputsProperties
                    clipId={clipId}
                    panelSize={panelSize}
                  />
                </TabsContent>
              ...
              {hasModel && (
                <TabsContent value="model-generation" className="min-w-0 m-0">
                  <ModelGenerationProperties clipId={clipId} />
                </TabsContent>
              )}
              {hasModel && (
                <TabsContent value="model-progress" className="min-w-0 m-0">
                  <ProgressPanel clipId={clipId} />
                </TabsContent>
              )}
              {hasModel && (
                <TabsContent value="model-architecture" className="min-w-0 m-0">
                  <ModelComponentsProperties clipId={clipId} />
                </TabsContent>
              )}
              {hasFrameInterpolate && (
                <TabsContent value="enhance" className="min-w-0 m-0">
                  <FrameInterpolateProperties clipId={clipId} />
                </TabsContent>
              )}
              {hasModel && (
                <TabsContent value="model-lora" className="min-w-0 m-0">
      ...
      {hasValidPreprocessor && (
        <div
          className="absolute bottom-0 left-0 right-0 p-5 bg-brand border-t border-brand-light/10"
        ...
        >
          <button
            ...
            onClick={
              preprocessor?.status === "running"
                ? handleStopPreprocessor
                : handleRunPreprocessor
            }
            className={cn(
              "w-full py-2.5 px-6 rounded-lg font-medium text-[12px] flex items-center disabled:opacity-60 text-brand-lighter disabled:cursor-not-allowed disabled:bg-brand-light/10 disabled:text-brand-light/50 justify-center gap-x-2 transition-all duration-200 shadow-lg hover:opacity-90",
              preprocessor?.status === "running"
                ? "bg-red-500"
      ...
      {hasModel && (
        <div
          className={cn(
            "absolute bottom-0 left-0 right-0  border-brand-light/5",
