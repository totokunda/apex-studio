...
const PropertiesPanel: React.FC<PropertiesPanelProps> = ({ order, defaultSize = 25, minSize = 25 }) => {
  ...
  >
   <div ref={containerRef} className="bg-brand-background rounded-lg h-full overflow-hidden min-w-0">
      {selectedClipIds.length > 0 || selectedPreprocessorId ? <ClipPropertiesPanel panelSize={panelSize} /> : <EmptyPropertiesPanel />}
   </div>
  </ResizablePanel>
  )
}
