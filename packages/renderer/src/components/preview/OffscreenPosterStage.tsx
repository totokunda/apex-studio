...
export type OffscreenPosterStageHandle = {
  toCanvas: (pixelRatio?: number, quality?: number) => Promise<HTMLCanvasElement | null>;
  toDataURL: (opts?: { mimeType?: string; quality?: number; pixelRatio?: number }) => Promise<string | null>;
};
...
const OffscreenPosterStage = forwardRef<OffscreenPosterStageHandle, Props>(({ width, height, frame, startFrame, endFrame, step = 1, pixelRatio = 1, onFrame, autoStart = true, inputId, clipId, clip: clipOverride, baseLongSide = BASE_LONG_SIDE, ratioOverride, offscreenFast = false, noDelay = false, onEnd }, ref) => {
  ...
  const [currentFrame, setCurrentFrame] = useState<number>(frame ?? startFrame ?? 0);
  ...
  const rectDims = useMemo(() => {
    ...
    const ratio = typeof ratioOverride === 'number' && isFinite(ratioOverride) && ratioOverride > 0 ? ratioOverride : editorRatio;
  ...
  const getApplicators = useCallback((id: string, frameOverride?: number) => {
  ...
  useEffect(() => {
    ...
    const hasRange = Number.isFinite(startFrame as number) && Number.isFinite(endFrame as number);
  ...
  }, [startFrame, endFrame, step, pixelRatio, onFrame, autoStart, noDelay, onEnd]);
      ...
      <Stage ref={stageRef} width={width} height={height} listening={false}>
        <Layer listening={false}>
          <Group x={view.x} y={view.y} scaleX={view.scale} scaleY={view.scale} listening={false}>
            <Rect x={0} y={0} width={rectWidth} height={rectHeight} fill={'#000000'} listening={false} />
            {toRender.map((clip) => {
              ...
              const effectiveGlobalFrame = clip.groupId ? (currentFrame + groupStart) : currentFrame;
              ...
              if (!overrideToUse?.groupId && overrideToUse?.type === 'image' || overrideToUse?.type === 'video') {
              ...
              switch (clip.type) {
                case 'video':
                  return <VideoPreview key={clip.clipId} {...(clip as any)} overrideClip={overrideToUse} rectWidth={rectWidth} rectHeight={rectHeight} applicators={applicators} overlap={true} inputMode={true} inputId={inputId} focusFrameOverride={currentFrame} currentLocalFrameOverride={currentFrame} offscreenFast={offscreenFast} />
                case 'image':
                  return <ImagePreview key={clip.clipId} {...(clip as any)} overrideClip={overrideToUse} rectWidth={rectWidth} rectHeight={rectHeight} applicators={applicators} overlap={true} inputMode={true} inputId={inputId} focusFrameOverride={currentFrame} currentLocalFrameOverride={currentFrame} />
                case 'shape':
