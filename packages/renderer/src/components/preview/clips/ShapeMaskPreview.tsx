...
const ShapeMaskPreview: React.FC<ShapeMaskPreviewProps> = ({
  ...
  const updateMaskBounds = useCallback((isResize: boolean = false) => {
    ...
    if (shapeType === 'ellipse') {
      ...
      if (shapeType === 'polygon' && typeof (shape as any).radius === 'function') {
      ...
      } else if (shapeType === 'star' && typeof (shape as any).outerRadius === 'function' && typeof (shape as any).innerRadius === 'function') {
        const effectiveOuter = (shape as any).outerRadius() * (shape.scaleX() || 1);
    ...
    const prevBounds = existingBounds as { width?: number; height?: number } | undefined;
    const widthForPosition = isResize ? actualWidth : (prevBounds?.width ?? actualWidth);
    const heightForPosition = isResize ? actualHeight : (prevBounds?.height ?? actualHeight);
    ...
    const targetClip = clipStoreState.clips.find(c => c.clipId === mask.clipId);
  ...
  const syncBlackShape = useCallback(() => {
    ...
    if (whiteShape && blackShape) {
      ...
      if (shapeType === 'rectangle') {
        ...
        if (typeof whiteShape.radius === 'function' && typeof blackShape.radius === 'function') {
        ...
        // Mirror white radii and scale for live alignment during resize
        if (typeof whiteShape.outerRadius === 'function' && typeof blackShape.outerRadius === 'function') {
          blackShape.outerRadius(whiteShape.outerRadius());
        }
        if (typeof whiteShape.innerRadius === 'function' && typeof blackShape.innerRadius === 'function') {
  ...
  const handleTransform = useCallback(() => {
    const whiteShape = shapeWhiteRef.current;
    const blackShape = shapeBlackRef.current;
    if (whiteShape && blackShape && (shapeType === 'polygon' || shapeType === 'star')) {
      ...
      if (!isRotate) {
        ...
        const parentScale = (Math.abs(parentScaleX) + Math.abs(parentScaleY)) / 2;
        ...
        if (shapeType === 'polygon' && typeof whiteShape.radius === 'function' && typeof blackShape.radius === 'function') {
          blackShape.radius(whiteShape.radius());
        } else if (shapeType === 'star') {
          if (typeof whiteShape.outerRadius === 'function' && typeof blackShape.outerRadius === 'function') {
            blackShape.outerRadius(whiteShape.outerRadius());
  ...
  const renderShape = useCallback((isWhite: boolean) => {
    const baseProps = {
      ...
      onMouseDown: isWhite ? (e: KonvaEventObject<MouseEvent>) => {
        if (!canInteract) return;
    ...
    switch (shapeType) {
      ...
      case 'polygon': {
        const radius = Math.min(actualWidth / Math.sqrt(3), actualHeight / 1.5);
  ...
  }, [x, y, width, height, rotation, animationOffset, shapeType, canInteract, isSelected, handleClick, handleTransform, handleTransformEnd, handleDragMove, handleDragEnd, groupRef]);
    ...
    <Group visible={tool==='mask'} ref={groupRef} clipX={0} clipY={0} clipWidth={rectWidth} clipHeight={rectHeight} onMouseOver={() => setIsOverMask(true)} onMouseOut={() => setIsOverMask(false)}>
      ...
      {renderShape(false)}
        ...
        <Transformer
          ...
          enabledAnchors={
            shapeType === 'star' ? ['top-left',  'top-right', 'bottom-left', 'bottom-right', ] :
            shapeType === 'polygon' ? ['top-left', 'top-right', 'bottom-left', 'bottom-right'] :
            ['top-left', 'top-center', 'top-right', 'bottom-left', 'bottom-right', 'middle-left', 'middle-right', 'bottom-center']
