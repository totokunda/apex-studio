...
export class FilterPreview extends BaseClipApplicator<FilterClipProps> {
    ...
    apply(
        ...
        try {
            ...
            const filteredCanvas = this.haldClutInstance.apply(canvas, filterPath, this.strength);
            ...
            if (ctx && filteredCanvas !== canvas) {
                ...
                if (canvas.width !== filteredCanvas.width || canvas.height !== filteredCanvas.height) {
    ...
    getFilterPath(useFullPath: boolean = true): string | null {
        const path = useFullPath ? (this.clip.fullPath || this.clip.smallPath) : (this.clip.smallPath || this.clip.fullPath);
        return path ?? null;
    }
}