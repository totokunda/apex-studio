...
export class LassoMask extends WebGLMaskBase {
    ...
    public apply(sourceCanvas: HTMLCanvasElement, mask: MaskClipProps, frame: number, clipTransform?: ClipTransform, maskTransform?: ClipTransform, debug?: { download?: boolean; annotateBounds?: boolean; filename?: string }): HTMLCanvasElement {
        ...
        const transformedLassoPoints = applyClipTransform(lassoPoints, clipTransform, maskTransform);
        ...
        this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, texWidth, texHeight, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, data);
        ...
        this.gl.uniform1f(this.gl.getUniformLocation(this.program, 'u_pointsTexWidth'), texWidth);
        ...
        if (debug?.annotateBounds) {
          this.debugAnnotateBoundsOnTop(sourceCanvas, { x: minX, y: minY, width: Math.max(0, maxX - minX), height: Math.max(0, maxY - minY) });
        }

        if (debug?.download) {
          const safeName = debug.filename ?? `lasso-mask-debug-f${frame}-n${numPoints}.png`;
