...
import OffscreenPosterStage, {
  OffscreenPosterStageHandle,
} from "@/components/preview/OffscreenPosterStage";
...
export type OffscreenSingleFrameOptions = Omit<
  OffscreenRangeOptions,
...
async function resolveDimensions(opts: {
  ...
  const clip =
    opts.clip ??
    (opts.clipId
      ? (store.getClipById(opts.clipId) as AnyClipProps | undefined)
      : undefined);
  ...
  if (clip && clip.type === "group") {
    ...
    return {
      width: rectW,
      height: rectH,
      baseLongSide: baseShortSide,
      ratioOverride: chosenRatio,
    };
  }
  ...
  const collectMediaClips = (c?: AnyClipProps): AnyClipProps[] => {
    ...
    if (c.type === "group") {
      ...
      return ids
        ...
        .filter(
          (mc): mc is AnyClipProps =>
            !!mc && (mc.type === "video" || mc.type === "image"),
        );
    }
  ...
  for (const mc of mediaClips) {
    const selectedSrc =
      (mc as any)?.preprocessors?.find((p: any) => p.status === "complete")
        ?.src ?? (mc as any)?.src;
    if (typeof selectedSrc !== "string" || !selectedSrc) continue;
    let info = getMediaInfoCached(selectedSrc);
    if (!info) {
      try {
        info = await getMediaInfo(selectedSrc);
      } catch {}
    }
    const w =
      (mc as any)?.mediaWidth ||
      info?.video?.displayWidth ||
      info?.image?.width ||
      0;
    const h =
      (mc as any)?.mediaHeight ||
    ...
    if (effectiveRatio === null) {
      ...
      effectiveRatio =
        typeof clipRatio === "number" && isFinite(clipRatio) && clipRatio > 0
          ? clipRatio
          : w / h;
    }
  }
...
async function waitForNonEmptyFrame(
  ref: React.RefObject<OffscreenPosterStageHandle | null>,
  pixelRatio: number,
  quality = 1.0,
  maxTries = 30,
...
export async function* renderOffscreenFrames(
  options: OffscreenRangeOptions,
): AsyncGenerator<OffscreenFrameResult> {
  const {
    startFrame,
    endFrame,
    step = 1,
  ...
  const { width, height, baseLongSide, ratioOverride } =
...
export async function renderOffscreenFrame(
  options: OffscreenSingleFrameOptions,
): Promise<HTMLCanvasElement | null> {
  const {
    frame,
    pixelRatio = 1,
