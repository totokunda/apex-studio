...
const AppearanceProperties: React.FC<AppearancePropertiesProps> = ({ clipId }) => {
    const clip = useClipStore((s) => s.getClipById(clipId)) as VideoClipProps | ImageClipProps | ShapeClipProps;
    const { updateClip } = useClipStore();
    const [spinning, setSpinning] = useState(false);
    const opacity = useMemo(() => clip?.transform?.opacity ?? 100, [clip?.transform?.opacity]);

    const setOpacity = (value: number) => {
        const numValue = typeof value === 'number' ? value : Number(value);
        if (isNaN(numValue) || !isFinite(numValue)) return;
        const clamped = Math.max(0, Math.min(100, numValue));
        updateClip(clipId, { transform: { ...clip?.transform!, opacity: clamped } });
    }
  ...
  return (
    <div className="flex flex-col gap-y-2 min-w-0">
      ...
      <div className="flex flex-row items-center justify-between mb-4">
        <h4 className="text-brand-light text-[12px] font-medium text-start">Appearance</h4>
        <span
          onClick={() => {
            updateClip(clipId, { transform: { ...clip?.transform!, opacity: 100, cornerRadius: 0 } });
          }}
      ...
      <div className="flex flex-col gap-y-2">
        ...
        <Input label="Stroke Width" value={(clip as ShapeClipProps)?.strokeWidth?.toString() ?? '0'} onChange={(value) => updateClip(clipId, { strokeWidth: Number(value) })} startLogo="W" canStep step={1} min={0} /> 
        </div>}
        </div>
        </div>
    </div>
