...
const AudioProperties: React.FC<AudioPropertiesProps> = ({ clipId }) => {
  ...
  return (
    <div className="flex flex-col gap-y-2">
      ...
      <div className="flex flex-col gap-y-2">
        <PropertiesSlider label="Volume" value={volume} onChange={setVolume}  suffix="dB" min={-60} max={20} step={1} />
        <PropertiesSlider label="Fade In" value={fadeIn} onChange={setFadeIn}  suffix="s" max={clipDuration} min={0} step={0.1} />
        <PropertiesSlider label="Fade Out" value={fadeOut} onChange={setFadeOut}  suffix="s" max={clipDuration} min={0} step={0.1} />
        </div>
        </div>
    </div>
  )
