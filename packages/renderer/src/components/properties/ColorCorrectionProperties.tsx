...
const ColorCorrectionProperties: React.FC<ColorCorrectionPropertiesProps> = ({ clipId }) => {
    const clip = useClipStore((s) => s.getClipById(clipId)) as VideoClipProps | ImageClipProps;
  ...
  return (
    <div className="p-4 px-5 min-w-0">
      <div className="flex flex-row items-center justify-between mb-4">
        <h4 className="text-brand-light text-[12px] font-medium text-start">Color Correction</h4>
        <span
          onClick={resetAll}
      ...
      <div className="flex flex-col gap-y-2">
        <PropertiesSlider label="Brightness" value={brightness} onChange={setBrightness} min={-100} max={100} step={1} toFixed={0}  />
        <PropertiesSlider label="Contrast" value={contrast} onChange={setContrast} min={-100} max={100} step={1} toFixed={0}  />
        <PropertiesSlider label="Hue" value={hue} onChange={setHue} min={-100} max={100} step={1} toFixed={0}  />
        <PropertiesSlider label="Saturation" value={saturation} onChange={setSaturation} min={-100} max={100} step={1} toFixed={0}  />
      </div>
    </div>
  )
}
