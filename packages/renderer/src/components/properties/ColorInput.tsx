...
const ColorInput: React.FC<ColorInputProps> = ({ value, onChange, label, percentValue, setPercentValue, labelClass, size = 'small' }) => {
  ...
  const validateAndConvertColor = (color: string): string | null => {
    ...
    const rgbMatch = parsedColor.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*[\d.]+)?\)$/);
    if (rgbMatch) {
  ...
  const handlePercentKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (tempPercentValue !== currentDisplay && !isNaN(numValue) && isFinite(numValue) && numValue >= 0 && numValue <= 100) {
        const oldValue = lastPercentValueRef.current;
        setPercentValue?.(numValue);
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
  ...
  <div className="flex flex-col items-start w-full gap-y-1 min-w-0">
    ...
    <ColorPicker 
       value={Color(value).alpha((percentValue ?? 100) / 100).rgb().array()}
       onChange={(value) => {
        ...
        if (newHex !== lastColorValueRef.current || newAlpha !== lastPercentValueRef.current) {
          onChange(newHex);
