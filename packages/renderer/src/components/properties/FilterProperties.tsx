...
const FilterProperties: React.FC<FilterPropertiesProps> = ({ clipId }) => {
  const clip = useClipStore((s) => s.getClipById(clipId)) as FilterClipProps | undefined
  ...
  return (
    <div className="flex flex-col gap-y-2 min-w-0 p-4 px-5">
      <h4 className="text-brand-light text-[12px] font-medium text-start mb-4">Filter</h4>
      <PropertiesSlider
        label="Intensity"
