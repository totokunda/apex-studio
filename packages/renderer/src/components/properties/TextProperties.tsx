...
import { RxLetterCaseLowercase, RxLetterCaseUppercase, RxTextAlignBottom, RxTextAlignMiddle, RxTextAlignTop } from 'react-icons/rx';
...
const fonts: FontItem[] = [
    ...
    { value: 'Montserrat', label: 'Montserrat', isPremium: true, isDownloaded: true },
    { value: 'Roboto', label: 'Roboto', isPremium: true, isDownloaded: true },
    { value: 'Open Sans', label: 'Open Sans', isPremium: true, isDownloaded: true },
    ...
    { value: 'Merriweather', label: 'Merriweather', isPremium: true, isDownloaded: true },
    { value: 'Playfair Display', label: 'Playfair Display', isPremium: true, isDownloaded: true },
...
const TextProperties: React.FC<TextPropertiesProps> = ({ clipId }) => {
    ...
    const TextTransformButtons = [
        ...
        { value: 'capitalize', icon: <RxLetterCaseCapitalize className="h-3 w-3" />,  },
    ]
    ...
    const setTextTransform = (value: string) => {
        updateClip(clipId, { textTransform: value as 'none' | 'uppercase' | 'lowercase' | 'capitalize', text: applyTextTransform(clip?.text ?? '', value) });
    }
    ...
    const toggleItalic = () => {
        ...
        updateClip(clipId, { fontStyle: currentStyle === 'italic' ? 'normal' : 'italic' });
    }

    const toggleUnderline = () => {
        const currentDecoration = clip?.textDecoration ?? 'none';
        updateClip(clipId, { textDecoration: currentDecoration === 'underline' ? 'none' : 'underline' });
    }

    return (
        <div className="flex flex-col gap-y-2 min-w-0">
        <div className='p-4 flex flex-col gap-y-5 px-5 min-w-0'>
        <h3 className="text-brand-light text-xs font-medium text-start">Basic</h3>
        <textarea
        ...
        <PropertiesSlider
            ...
            <div className='flex flex-row bg-brand divide-x divide-brand-light/10 rounded w-full'>
                <button 
                    onClick={toggleBold}
                    className={cn('flex items-center p-1.5 justify-center w-full text-brand-light/70 cursor-pointer hover:text-brand-light hover:bg-brand-light/10 duration-200 transition-all rounded-l',
                        (clip?.fontWeight ?? 400) >= 700 && 'bg-brand-light/10 text-brand-light'
                    )}
                >
                ...
                <button 
                    ...
                    className={cn('flex items-center p-1.5 justify-center w-full text-brand-light/70 cursor-pointer hover:text-brand-light hover:bg-brand-light/10 duration-200 transition-all',
                        (clip?.fontStyle ?? 'normal') === 'italic' && 'bg-brand-light/10 text-brand-light'
                    )}
                >
                ...
                <button 
                    ...
                    className={cn('flex items-center p-1.5 justify-center w-full text-brand-light/70 cursor-pointer hover:text-brand-light hover:bg-brand-light/10 duration-200 transition-all rounded-r',
                        (clip?.textDecoration ?? 'none') === 'underline' && 'bg-brand-light/10 text-brand-light'
                    )}
                >
            ...
            <ButtonList buttons={AlignmentButtons} selected={clip?.textAlign ?? 'left'} onSelect={setAlignment} />
            </div>
            <div className='flex flex-row gap-x-2 items-center'>
            <h4 className='text-brand-light text-[10.5px] text-start w-1/3'>
                Vertical Align
            </h4>
            <ButtonList buttons={VerticalAlignmentButtons} selected={clip?.verticalAlign ?? 'top'} onSelect={setVerticalAlignment} />
            </div>
            <div className='flex flex-row gap-x-2 items-center'>
            <h4 className='text-brand-light text-[10.5px] text-start w-1/3'>
                Text Transform
