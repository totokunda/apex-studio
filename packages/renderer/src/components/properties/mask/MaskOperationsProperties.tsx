...
const MaskOperationsProperties: React.FC<MaskOperationsPropertiesProps> = ({ mask, clipId }) => {
  const getClipById = useClipStore((s) => s.getClipById);
  const updateClip = useClipStore((s) => s.updateClip);



  const updateMask = useCallback((updates: Partial<MaskClipProps>) => {
    const currentClip = getClipById(clipId);
    if (!currentClip || (currentClip.type !== 'video' && currentClip.type !== 'image')) return;
          ...
          <div className="flex flex-col gap-y-2">
            ...
            <div className="flex flex-row flex-wrap gap-x-2 gap-y-2 mb-1.5">
              <button onClick={handleBinaryMode}>
                <span className="text-brand-light text-[10px] font-medium px-3 py-1 rounded-md border border-brand-light/10 bg-brand hover:bg-brand-light/10 transition-all duration-200">Binary</span>
              </button>
              <button onClick={handleTransparentForeground}>
                <span className="text-brand-light text-[10px] font-medium px-3 py-1 rounded-md border border-brand-light/10 bg-brand hover:bg-brand-light/10 transition-all duration-200">Transparent Foreground</span>
              </button>
              <button onClick={handleTransparentBackground}>
                <span className="text-brand-light text-[10px] font-medium px-3 py-1 rounded-md border border-brand-light/10 bg-brand hover:bg-brand-light/10 transition-all duration-200">Transparent Background</span>
              </button>
              <button onClick={handleGrayMode}>
                <span className="text-brand-light text-[10px] font-medium px-3 py-1 rounded-md border border-brand-light/10 bg-brand hover:bg-brand-light/10 transition-all duration-200">Gray</span>
              </button>
              <button onClick={handleInvertedMode}>
                <span className="text-brand-light text-[10px] font-medium px-3 py-1 rounded-md border border-brand-light/10 bg-brand hover:bg-brand-light/10 transition-all duration-200">Binary Inverted</span>
              </button>
              <button onClick={handleSemiTransparentMode}>
                <span className="text-brand-light text-[10px] font-medium px-3 py-1 rounded-md border border-brand-light/10 bg-brand hover:bg-brand-light/10 transition-all duration-200">Semi-Transparent</span>
              </button>
            </div>
            ...
            <div className="flex flex-col gap-y-3">
              ...
              <div className="flex flex-col gap-y-2">
                <div className="flex flex-row items-center justify-between">
                  <label className="text-brand-light text-[11px] font-medium">Masked Region</label>
                ...
                {(mask.maskColorEnabled ?? true) && (
                 ...
                 <button
                    onClick={() => updateMask({ maskColorEnabled: !(mask.maskColorEnabled ?? true) })}
                    className={cn(
              ...
              <div className="flex flex-col gap-y-2">
                <div className="flex flex-row items-center justify-between">
                  <label className="text-brand-light text-[11px] font-medium">Background</label>
                ...
                {(mask.backgroundColorEnabled ?? true) && (
                 ...
                 <button
                    onClick={() => updateMask({ backgroundColorEnabled: !(mask.backgroundColorEnabled ?? true) })}
                    className={cn(
                      "px-6 py-1.5 rounded-full text-[9px] font-medium transition-all duration-200 w-fit",
                      mask.backgroundColorEnabled ?? true
                        ? "bg-brand-light/10 text-brand-light/80 border border-brand-light/20 hover:bg-brand-light/20"
