...
const MaskPropertiesPanel: React.FC<MaskPropertiesPanelProps> = ({ clipId }) => {
  ...
  const clip = useClipStore((s) => s.getClipById(clipId)) as PreprocessorClipType | undefined;
  ...
  const activeMasks = useMemo(() => {
    ...
    return masks.filter((m) => {
      const keyframes = m.keyframes instanceof Map ? m.keyframes : (m.keyframes as Record<number, any>);
  ...
  const mask = useMemo(() => {
    ...
    const masks = clip.masks || [];
    let mask = masks.find((m: MaskClipProps) => m.id === selectedMaskId) as MaskClipProps | null;
    if (!mask) {
      ...
      {activeMasks.length > 0 && (
        <div className="flex flex-row gap-x-2 items-center justify-start px-4 pt-4 flex-wrap gap-y-2">
          {activeMasks.map((m: MaskClipProps, idx: number) => (
            <div key={m.id} onClick={() => setSelectedMaskId(m.id)} className={cn("flex flex-row group relative items-center justify-between px-3 py-1 cursor-pointer rounded-full w-fit border border-brand-light/10 bg-brand", m.id === selectedMaskId && "bg-brand-light/10 border-brand-light/20")}>
              <p className="text-brand-light text-[11px] font-medium">{m.tool.charAt(0).toUpperCase() + m.tool.slice(1)} Mask {idx + 1}</p>
              <div onClick={() => handleRemoveMask(m.id)} className="text-brand-light/50 hover:text-red-400 transition-colors absolute -right-1 -top-1 group-hover:block hidden p-0.5 border bg-brand border-brand-light/10 rounded-full cursor-pointer">
