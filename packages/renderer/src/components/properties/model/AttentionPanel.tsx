...
const AttentionPanel: React.FC<AttentionPanelProps> = ({ clipId }) => {
  ...
  const options = useMemo(() => {
    const detail = (clip?.manifest?.spec?.attention_types_detail || []) as Array<{ name: string; label?: string; description?: string }>
  ...
  useEffect(() => {
    ...
    if (!curr || !names.includes(String(curr))) {
      ...
      const nextSelected = { ...(clip.selectedComponents || {}), attention: next };
  ...
  const selectedOption = useMemo(() => options.find((o) => String(o.name) === String(selectedName)), [options, selectedName]);
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <div className="flex flex-col text-left min-w-0 w-0 flex-1 gap-y-0.5 overflow-hidden">
              <span className="block truncate font-medium">{selectedOption?.label || selectedOption?.name || 'Select attention'}</span>
              {selectedOption?.description && (
                <span className="block truncate text-[10px] text-brand-light/70 font-normal">{selectedOption.description}</span>
              )}
            </div>
        ...
        >
          <Command className="bg-brand-background">
            <CommandInput placeholder="Search attention" className="text-brand-light placeholder:text-brand-light/40 text-[11px]" />
            <CommandList>
              <CommandEmpty className="text-brand-light/40 p-3 text-xs">No attention found.</CommandEmpty>
                ...
                {options.map((opt) => {
                  ...
                  return (
                    <CommandItem
                      ...
                      value={opt.name}
                      onSelect={(val) => {
                        const picked = options.find((o) => String(o.name) === String(val));
                        const next = picked ? { name: picked.name } : { name: val };
                        const nextSelected = { ...(clip?.selectedComponents || {}), attention: next };
                        updateClip(clipId, { selectedComponents: nextSelected } as any);
                        setOpen(false);
                      }}
                    ...
                    >
                      <div className="h-4 w-4 mt-0.5">
                        {isSelected ? <Check className="h-4 w-4 text-brand-lighter" /> : <span className="inline-block h-4 w-4" />}
                      </div>
                      <div className="flex flex-col min-w-0 max-w-full overflow-hidden">
                        <span className="block text-brand-light text-[11.5px] font-medium truncate">{opt.label || opt.name}</span>
                        {opt.description && (
                          <span className="block text-[10px] text-brand-light/70 truncate">{opt.description}</span>
                        )}
                      </div>
                    </CommandItem>
                  );
