...
const ModelComponentsProperties = ({ clipId }: ModelComponentsPropertiesProps) => {
    ...
    const normalizeModelPaths = (c: ManifestComponent): ManifestComponentModelPathItem[] => {
    ...
    useEffect(() => {
        ...
        components.forEach((comp) => {
            ...
            if (!hasValid) {
                const first = items[0];
                setSelection(key, { path: first.path, variant: first.variant, precision: first.precision, type: first.type });
            }
        });
    ...
    const renderModelPathSection = (comp: ManifestComponent) => {
        ...
        return (
            <div className="flex flex-col gap-2 justify-start items-start w-full p-2 ">
                <div className="text-brand-light text-[11px] font-medium w-full text-left">{formatCompLabel(comp)}</div>
                {downloaded.length === 0 ? (
                    <div className="text-[10.5px] text-brand-light/60 ">No component downloaded.</div>
                ) : (
                    <div className="flex flex-col gap-1.5 w-full min-w-0 justify-start items-start ">
                        {downloaded.map((it, idx) => {
                            const label = formatComponentName(it.variant || (idx === 0 ? 'Default' : it.path.split('/').pop() || it.path));
                            const subtitle = [it.precision?.toUpperCase(), it.type?.toLowerCase() === 'gguf' ? 'GGUF' : formatComponentName(it.type || '')].filter(Boolean).join(' • ');
                            const isSelected = String(selectedPath) === String(it.path);
                            return (
                                <button
                                    key={`${it.path}-${idx}`}
                                    type="button"
                                    onClick={() => setSelection(key, { path: it.path, variant: it.variant, precision: it.precision, type: it.type })}
                                    className={cn(
                            ...
                            <div className="flex items-start justify-between gap-2 w-full min-w-0 overflow-hidden">
                                <div className="min-w-0 w-0 basis-0 flex-1 overflow-hidden">
                                    <div className="text-brand-light text-[11px] font-medium truncate">{label}</div>
                                    <div className="text-[10px] text-brand-light/70 mt-0.5 font-mono min-w-0 truncate">{it.path}</div>
                                    
                                </div>
                                        {isSelected && <div className="shrink-0 p-[2.5px] rounded-full bg-green-500/20 border-green-500/40 border ">
                                    ...
                                    {subtitle && (
                                        <div className="text-[10px] text-brand-light/70  truncate flex flex-row items-center gap-x-1 justify-between mt-1">
                                            {it.type && <span className="text-brand-light/90 font-medium">{it.type?.toLowerCase() === 'gguf' ? 'GGUF' : formatComponentName(it.type || '')}</span>}
                                            {it.precision && <span className="text-brand-light/90 font-mono">{it.precision?.toUpperCase()}</span>}
                                        </div>
                                    )}
                                </button>
                            );
        ...
        <div className="flex flex-col p-4 justify-start items-stretch w-full">
            <div className="flex flex-col gap-3 w-full">
                {components.map((comp, index) => {
                    if (comp.model_path) {
                        return (
                            <div key={`${comp.type}-${index}`} className="w-full">{renderModelPathSection(comp)}</div>
                        );
                    }
                    return null;
                })}
