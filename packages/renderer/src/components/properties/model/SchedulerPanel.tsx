...
const SchedulerPanel: React.FC<SchedulerPanelProps> = ({ clipId }) => {
  ...
  const options = useMemo(() => {
    ...
    return unique as Array<{ name: string; label?: string; description?: string; base?: string; config_path?: string }>;
  }, [clip]);
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <div className="flex flex-col text-left min-w-0 w-0 flex-1 gap-y-0.5 overflow-hidden">
              <span className="block truncate font-medium">{selectedOption?.label || selectedOption?.name || 'Select scheduler'}</span>
        ...
        >
          <Command className="bg-brand-background">
            <CommandInput placeholder="Search scheduler" className="text-brand-light placeholder:text-brand-light/40 text-[11px]" />
            <CommandList>
              <CommandEmpty className="text-brand-light/40 p-3 text-xs">No scheduler found.</CommandEmpty>
                  ...
                  return (
                    <CommandItem
                      ...
                      onSelect={(val) => {
                        ...
                        const next = picked ? { name: picked.name, base: picked.base, config_path: picked.config_path } : { name: val };
                        const nextSelected = { ...(clip?.selectedComponents || {}), scheduler: next };
                        updateClip(clipId, { selectedComponents: nextSelected } as any);
                        setOpen(false);
                      }}
