...
const BooleanInput: React.FC<BooleanInputProps> = ({ label, description, value, onChange }) => {
  ...
  return (
    <div className="flex flex-row items-center w-full gap-x-2 min-w-0 relative mt-1">
      <div className="flex items-center gap-1.5">
        <label className="text-brand-light text-[10.5px] text-start font-medium">{label}</label>
        {description && (
          <Tooltip>
      ...
      <button
        ...
        className={cn(
          ...
          isOn ? 'bg-blue-600' : 'bg-brand-background border border-brand-light/10'
        )}
      >
        <span
          className={cn(
