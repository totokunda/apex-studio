...
const PopoverImage: React.FC<PopoverImageProps> = ({ value, onChange }) => {
    ...
    return (
        <PopoverContent 
            ...
            align='start'
            sideOffset={20} 
            className='p-2 dark h-full flex flex-col gap-y-3 w-96 border border-brand-light/10 rounded-[7px] font-poppins' 
            onOpenAutoFocus={() => { isUserInteractingRef.current = true; }} onCloseAutoFocus={() => { isUserInteractingRef.current = false; }}>
                <Tabs value={selectedTab} onValueChange={(value) => setSelectedTab(value as 'timeline' | 'library')}>
                    ...
                    <ScrollArea  className='w-full h-96 px-3'>
                        <div className='w-full h-full grid grid-cols-2 gap-3'>
                            {filteredMediaItems.map((media) => (
                                <div key={media.name} className='w-full flex flex-col items-center justify-center gap-y-1.5 cursor-pointer group relative'>
                                    <div className='relative'>
                                    <div className='absolute top-0 left-0 w-full h-full bg-brand-background-light/50 backdrop-blur-sm rounded-md z-20  group-hover:opacity-100 opacity-0 transition-all duration-200' />
...
const ImageInput: React.FC<ImageInputProps> = ({ label, description, value, onChange, placeholder, defaultValue }) => {
    const isUserInteractingRef = useRef(false);
  return (
    ...
    <Popover>
        ...
        <PopoverImage value={value} onChange={onChange} />
    </Popover>
    </div>
    </div>
    </Droppable>
