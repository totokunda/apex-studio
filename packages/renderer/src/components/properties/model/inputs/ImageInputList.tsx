...
import { LuCircleX, LuImageOff, LuImagePlus, LuTrash, LuX } from 'react-icons/lu';
...
const ImageInputList:React.FC<ImageInputListProps> = ({ label, description, inputId, value = [], onChange, clipId, panelSize, min: _min, max }) => {
  const handleAdd = () => {
  ...
  const enableScroll = typeof panelSize === 'number' ? panelSize < totalNeededWidth : true;
  ...
  return (
    <div className='w-full pr-1'>
        ...
        <ScrollArea
          className="w-full"
          style={{
            maxWidth: typeof panelSize === 'number' && panelSize > 0 ? panelSize : undefined,
          }}
        >
          <div
          ...
          >
            {value.length === 0 && (
              <div className="w-48 h-48 flex flex-col items-center justify-center gap-y-2 py-3 opacity-70 bg-brand rounded-md">
                ...
                <span className="text-brand-light text-[11.5px] font-medium">No images added</span>
              </div>
            )}
            {value.map((item, index) => (
              <div key={`${inputId}-${index}`} className="w-48 flex-shrink-0 relative">
              <button onClick={() => handleItemRemove(index)} className="absolute z-[51] shadow-lg -top-1 -right-1 rounded-full w-5 h-5 flex items-center justify-center  bg-brand-background-dark border border-red-500 hover:bg-brand-background transition-all duration-200">
            ...
            {canAddMore && (
              ...
              >
                ...
                <span className="text-brand-light text-[11.5px] font-medium">Add Image</span>
              </button>
            )}
          </div>
        </ScrollArea>
