...
import { LuChevronDown, LuChevronLeft, LuChevronRight, LuChevronUp, LuInfo, LuPlus, LuX } from 'react-icons/lu';
...
const NumberListInput: React.FC<NumberListInputProps> = ({
  ...
  const canAddMore = typeof maxItems === 'number' ? numbers.length < maxItems : true;
  ...
  const incrementTemp = () => {
    const curr = normalizeNumber(tempValue || (numbers.length ? String(numbers[numbers.length - 1]) : '0'), valueType);
    if (curr == null) return;
    const next = clampNumber(curr + (step ?? 0), min, max);
    setTempValue(valueType === 'integer' ? String(Math.trunc(next)) : String(next));
  };
  ...
  return (
    <div className="flex flex-col items-start w-full gap-y-1 min-w-0">
      <div className="flex items-center gap-1.5 flex-row justify-between w-full">
        ...
        {description && (
         ...
         {typeof maxItems === 'number' && (
          <span className="text-[9px] text-brand-light/60 ml-auto">{numbers.length}/{maxItems}</span>
        )}
      </div>
      ...
      <div className="relative w-full flex flex-row items-center min-w-0">
        {startLogo && (
          <span className="text-brand-light/50 font-medium text-[11px] absolute left-2 top-1/2 -translate-y-1/2">{startLogo}</span>
        )}
        <input
          className={cn(
