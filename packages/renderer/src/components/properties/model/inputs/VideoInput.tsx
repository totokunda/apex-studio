...
import { AnyClipProps, VideoClipProps } from '@/lib/types';
...
export type VideoSelection = VideoSelectionMedia | VideoSelectionClip | null;
      ...
      <ScrollArea className="w-full h-96">
        <div className="w-full h-full grid grid-cols-2 gap-3">
          {filteredMediaItems.map((media) => {
            const isSelected = value?.kind === 'media' && value.assetUrl === media.assetUrl;
...
const VideoInput: React.FC<VideoInputProps> = ({ label, description, inputId, value, onChange, clipId, panelSize }) => {
  ...
  const valueKind = value?.kind ?? null;
  const valueAssetUrl = valueKind === 'media' ? (value as VideoSelectionMedia)?.assetUrl ?? null : null;
  const valueClipId = valueKind === 'clip' ? (value as VideoSelectionClip)?.clipId ?? null : null;
  ...
  const selectionKey = useMemo(() => {
    ...
    if (valueKind === 'media' && valueAssetUrl) return `media:${valueAssetUrl}`;
    if (valueKind === 'clip' && valueClipId) return `clip:${valueClipId}`;
    return 'null';
  }, [valueKind, valueAssetUrl, valueClipId]);
  ...
  useEffect(() => {
    ...
    const signature =
      !valueKind
        ? 'null'
        : `${selectionKey}:${requestedStartFrame}:${requestedEndFrame}`;
    ...
    if (valueKind === 'media' && valueAssetUrl) {
      applyRangeAndTimeline(
        requestedStartFrame,
        requestedEndFrame,
        {
          duration: Math.max(requestedEndFrame, 1),
          fps: DEFAULT_FPS,
          focus: requestedStartFrame,
        }
      );
      (async () => {
        try {
          const info = await getMediaInfo(valueAssetUrl);
          if (cancelled) return;
          const durationFrames = Math.max(1, Math.floor((info?.duration || 0) * fps));
          const clipForTimeline: VideoClipProps = {
            type: 'video',
            clipId: `media:${valueAssetUrl}`,
            src: valueAssetUrl,
            startFrame: 0,
            endFrame: durationFrames,
            preprocessors: [],
          ...
          setMediaClip((prev) => {
            if (
              prev &&
              prev.clipId === clipForTimeline.clipId &&
              (prev.startFrame ?? 0) === (clipForTimeline.startFrame ?? 0) &&
              (prev.endFrame ?? 0) === (clipForTimeline.endFrame ?? 0)
            ) {
              return prev;
            }
            return clipForTimeline;
          });
    ...
    } else if (valueKind === 'clip' && valueClipId) {
      const clip = getClipById(valueClipId) as AnyClipProps | undefined;
      ...
      setMediaClip((prev) => {
        if (
          prev &&
          prev.clipId === projectedClip.clipId &&
          (prev.startFrame ?? 0) === (projectedClip.startFrame ?? 0) &&
          (prev.endFrame ?? 0) === (projectedClip.endFrame ?? 0)
        ) {
          return prev;
        }
        return projectedClip;
      });
      const desiredStart = Math.max(0, Math.min(duration - 1, requestedStartFrame));
      const desiredEnd = Math.max(desiredStart + 1, Math.min(duration, requestedEndFrame));
      applyRangeAndTimeline(desiredStart, desiredEnd, {
        duration,
        fps: DEFAULT_FPS,
        focus: desiredStart,
  ...
  const previewClip = useMemo<AnyClipProps | null>(() => {
    if (!valueKind) return null;
    if (valueKind === 'media' && valueAssetUrl) {
      const [, persistedEnd] = getTimelineDuration(inputId);
      const fallbackEnd = Math.max(1, persistedEnd || rangeEndForInput);
      const fallback: VideoClipProps = {
        type: 'video',
        clipId: `media:${valueAssetUrl}`,
        src: valueAssetUrl,
        startFrame: 0,
        endFrame: fallbackEnd,
        preprocessors: [],
        masks: [],
      } as VideoClipProps;
      return mediaClip ?? (fallback as AnyClipProps);
    }
    if (valueKind === 'clip' && valueClipId) {
    ...
    return null;
  }, [valueKind, valueAssetUrl, valueClipId, mediaClip, getClipById, rangeEndForInput, getTimelineDuration, inputId]);
            ...
            <PopoverTrigger className="w-full">
              <div
                ...
                className={cn(
                  ...
                  valueKind ? '' : 'border-dashed',
                  valueKind ? '' : 'p-4 border-brand-light/10 border bg-brand-background-light/50'
                )}
              >
                {isOverDropZone && (
