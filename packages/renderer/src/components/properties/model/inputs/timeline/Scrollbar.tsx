...
const Scrollbar:React.FC<ScrollbarProps> = ({
    ...
    const assetStore = useAssetControlsStore();
    const scrollbarHeight = SCROLLBAR_HW;
    const {totalTimelineFrames, timelineDuration, zoomLevel, focusFrame} = useAssetControlsStore();
    const horizontalScrollStateRef = useRef({ startX: 0, lastX: 0, startFrame: 0, fractionalFrames: 0 });
    ...
    const {  scrollbarWidth, maxScrollbarX} = useMemo(() => {
        ...
        const scrollbarWidth = Math.max(20, (visibleDuration / totalTimelineFrames) * stageWidth);
    ...
    }, [stageWidth, stageHeight, zoomLevel, timelineDuration, totalTimelineFrames]);
    ...
    useEffect(() => {
        if (!isDraggingRef.current) {
            ...
            const scrollbarPosition = maxScrollbarX > 0 && scrollableFrames > 0
    ...
    return (
       <Rect
          ...
          onDragStart={(e) => {
            ...
            // No playback state in asset controls; nothing to pause
          }}
          onDragMove={(e) => {
            const x = e.target.x();
            setScrollbarX(x);
          ...
          onDragEnd={(e) => {
            ...
            // No playback state to restore in asset controls
          }}
          />
    )
  }
