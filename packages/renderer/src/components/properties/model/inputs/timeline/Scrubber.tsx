...
const AssetScrubControl:React.FC<AssetScrubControlProps> = ({stageHeight, stageWidth}) => {
    const { timelineDuration, fps, zoomLevel, focusFrame, setFocusFrame, setFocusAnchorRatio } = useAssetControlsStore();
    ...
    return(
        ...
        >
            <div className='flex flex-col items-center'>
                <div className={cn("w-2.5 h-3 rounded-t-sm rounded-b-[6px] border-2 border-white shadow-md bg-white")}></div>
                <div style={{height:stageHeight}} className={cn('rounded-b-full flex items-center w-[0.08rem] bg-white shadow-md')}></div>
            </div>
        </div>
    )
}
