...
const TimelineSearch: React.FC<TimelineSearchProps> = ({ types, width = 580, height = 300, excludeClipId = null }) => {
    ...
    const timelines = useMemo(() => {
      ...
      const typeFiltered = allowedTimelineTypes.size === 0
        ? originalTimelines
    ...
    const contentHeight = useMemo(() => {
        return timelines.reduce((acc, timeline) => {
            return Math.max(acc, (timeline.timelineY || 0) + (timeline.timelineHeight || 0));
        }, 0);
    }, [timelines]);
    ...
    <>
     ...
     <TimelineZoom />
          ...
          <Stage 
            ...
            >
                <Layer ref={timelinesLayerRef} visible={hasClips} y={-clampedScroll}>
                    {timelines.map((timeline, index) => (
                        <Timeline assetMode excludeClipId={excludeClipId || null} cornerRadius={4} key={timeline.timelineId} scrollY={clampedScroll} timelinePadding={timeline.timelinePadding} index={index} type={timeline.type} muted={timeline.muted} hidden={timeline.hidden} timelineWidth={dimensions.stageWidth} timelineY={timeline.timelineY} timelineHeight={timeline.timelineHeight} timelineId={timeline.timelineId} />
                    ))} 
                </Layer>
            ...
            {hasClips && (
              <AssetScrubControl stageHeight={dimensions.stageHeight - 24} stageWidth={dimensions.stageWidth} />
            )}
            </div>
