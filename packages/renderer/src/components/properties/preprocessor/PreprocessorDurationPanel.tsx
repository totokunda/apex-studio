...
const PreprocessorDurationPanel: React.FC<PreprocessorDurationPanelProps> = ({ preprocessorId }) => {
  const preprocessor = useClipStore((s) => s.getPreprocessorById(preprocessorId))!;
  const updatePreprocessor = useClipStore((s) => s.updatePreprocessor);
  const clip = useClipStore((s) => s.getClipFromPreprocessorId(preprocessorId))! as PreprocessorClipType;
  ...
  const setStartFrame = (value: number) => {
    const { isValid } = validatePreprocessorFrames(value, endFrame ?? 0, preprocessorId, clip.preprocessors, clipDuration);
    if (!isValid) {
      return;
    }
    updatePreprocessor(clip?.clipId ?? '', preprocessorId, { startFrame: value });
  }

  const setEndFrame = (value: number) => {
    const { isValid } = validatePreprocessorFrames(startFrame ?? 0, value, preprocessorId, clip.preprocessors, clipDuration);
    if (!isValid) {
      return;
    }
  ...
  const startFrameMax = Math.max(0, (clip?.endFrame ?? 0) - (clip?.startFrame ?? 0) - 1);
  const endFrameMax = Math.max(0, (clip?.endFrame ?? 0) - (clip?.startFrame ?? 0));
