...
const PreprocessorInfoPanel: React.FC<PreprocessorInfoPanelProps> = ({ preprocessor }) => {
  const hasParameters = preprocessor.preprocessor.parameters && preprocessor.preprocessor.parameters.length > 0;
  ...
  useEffect(() => {
    if (mainDescRef.current && preprocessor.preprocessor.description && !isMainDescExpanded) {
      const checkTruncation = mainDescRef.current.scrollHeight > mainDescRef.current.clientHeight;
      if (checkTruncation !== isMainDescTruncated) {
        setIsMainDescTruncated(checkTruncation);
      }
    }
  }, [preprocessor.preprocessor.description, isMainDescTruncated, isMainDescExpanded]);
  ...
  return (
    <div className="p-6 flex flex-col gap-y-4">
      <div className="flex flex-col gap-y-2">
        <div className="flex flex-col gap-y-1">
          <span className="text-brand-lighter text-[14px] font-semibold text-start">{preprocessor.preprocessor.name}</span>
          <span className="text-brand-light/80 text-[11px] text-start">{preprocessor.preprocessor.category}</span>
        </div>
        <div className="flex flex-col gap-y-1">
          <span 
            ref={mainDescRef}
      ...
      {hasParameters && (
        <div className="flex flex-col gap-y-3 border-t border-brand-light/10 pt-4">
          ...
          >
            ...
            <h4 className="text-brand-lighter text-[12px] font-semibold text-start">Parameters</h4>
          </button>
          
          {isParametersExpanded && (
            <div className="flex flex-col gap-y-3">
              {preprocessor.preprocessor.parameters!.map((param, index) => (
                <div key={index} className="flex flex-col gap-y-2 p-3 rounded-lg bg-brand-light/5 border border-brand-light/10">
