...
const PreprocessorParametersPanel: React.FC<PreprocessorParametersPanelProps> = ({ preprocessor}) => {
  ...
  const storedPreprocessor = useClipStore((s) => s.getPreprocessorById(preprocessor.id));
    ...
    if (preprocessor.preprocessor.parameters) {
      preprocessor.preprocessor.parameters.forEach(param => {
        ...
        if (storedValues[param.name] !== undefined) {
          ...
          params[param.name] = getDefaultValueForType(param.type, param.options);
        }
      });
    }
    return params;
  }, [preprocessor.preprocessor.parameters, JSON.stringify(storedPreprocessor?.values)]);

  const [paramValues, setParamValues] = useState<Record<string, any>>(initialParams);
          ...
          <div key={index} className="flex flex-col gap-y-3 ">
            ...
            <div className="mt-2">
              {param.type === 'int' && (
                <PropertiesSlider
                  label=""
                  value={paramValues[param.name] ?? param.default ?? param.min ?? 0}
              ...
              {param.type === 'category' && param.options && param.options.length > 0 && (
                <CategorySelector
                  value={String(paramValues[param.name] ?? param.default ?? param.options[0].value)}
                  onChange={(value) => handleParamChange(param.name, value)}
