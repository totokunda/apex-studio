...
const SystemMemoryMenu: React.FC = () => {
  ...
  const usageLine = (used?: number, total?: number, percent?: number) => {
    ...
    const p = Number.isFinite(percent || NaN) ? ` (${Math.round(percent as number)}%)` : '';
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger className='text-brand-light/90 dark h-[34px] relative flex items-center space-x-2 w-32 px-3 font-medium border border-brand-light/10 hover:text-brand-light bg-brand hover:bg-brand-light/10 rounded-[6px] py-[7px] transition-all duration-300 cursor-pointer'>
        ...
        <span className='ml-1 text-[11px] text-brand-light/60'>{Math.round(summaryPercent)}%</span>
      </PopoverTrigger>
      <PopoverContent align='end' className='bg-brand-background/90 backdrop-blur-md border border-brand-light/10 rounded-[8px] p-4 font-poppins' style={{ width: contentWidth }}>
        {data ? (
          <div className='flex flex-col gap-3'>
            <div className='flex items-center justify-between'>
              <div className='text-[11px] uppercase tracking-wide text-brand-light/80 font-medium'>System</div>
              <div className='text-[11px] text-brand-light/80 font-medium'> {data.device_type.toUpperCase()}</div>
            </div>
            <div className='flex items-stretch justify-between gap-4'>
              {data.unified ? (
                <div className='flex-1 flex flex-col items-center'>
                  <SemiGauge value={data.unified.percent} label='Unified Memory' />
            ...
            {data.unified ? (
              <div className='mt-1'>
                <div className='text-[11px] text-brand-light/90 font-medium mb-1'>Breakdown</div>
                <div className='grid grid-cols-2 gap-y-1 gap-x-3 text-[11px]'>
                  <div className='text-brand-light/70'>Physical Memory</div>
                  <div className='text-right text-brand-light'>{fmt(data.unified.total)}</div>
                  <div className='text-brand-light/70'>Memory Used</div>
                  <div className='text-right text-brand-light'>{usageLine(data.unified.used, data.unified.total)}</div>
                  <div className='text-brand-light/70'>Cached Files</div>
                  <div className='text-right text-brand-light'>{fmt((data.unified as any).details?.cached_files)}</div>
                  <div className='text-brand-light/70'>Wired Memory</div>
                  <div className='text-right text-brand-light'>{fmt((data.unified as any).details?.wired)}</div>
                  <div className='text-brand-light/70'>Compressed</div>
                  <div className='text-right text-brand-light'>{fmt((data.unified as any).details?.compressed)}</div>
                  <div className='text-brand-light/70'>Swap Used</div>
                  <div className='text-right text-brand-light'>{fmt((data.unified as any).details?.swap_used)}</div>
                </div>
              </div>
            ) : data.gpu ? (
              <div className='mt-1'>
                <div className='text-[11px] text-brand-light/70 mb-1'>GPU Adapters</div>
                <div className='space-y-1'>
                  {data.gpu.adapters.map((a) => (
                    <div key={a.index} className='flex items-center justify-between text-[11px]'>
                      <div className='text-brand-light/70'>GPU {a.index}</div>
                      <div className='text-brand-light'>{usageLine(a.used, a.total, a.percent)}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className='text-[10px] text-brand-light/40 mt-1'>Updates every 2s</div>
          </div>
        ) : (
