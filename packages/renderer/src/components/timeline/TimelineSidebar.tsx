...
import { LuAudioWaveform, LuPaintbrush, LuShapes, LuType, LuBox } from "react-icons/lu";
...
const TimelineSidebarItem:React.FC<TimelineProps & {
    isCollapsed: boolean;
    clampedScroll: number;
  }> = ({ isCollapsed, type, timelineHeight, timelineY, timelineId, clampedScroll }) => {
  ...
  <div style={{
    ...
    {!isCollapsed && (
        ...
        {(type !== 'audio') && <button className='cursor-pointer' onClick={() => {
            ...
            {timelineHidden ? <IoEyeOff className='w-3 h-3 text-brand-light hover:text-brand-lighter' /> : <IoEye className='w-3 h-3 text-brand-light hover:text-brand-lighter' />}
        </button>}
        </div>
    )}
  </div>
  ...
  return (
    ...
    >
      ...
      {timelines.map((timeline) => (
        <TimelineSidebarItem key={timeline.timelineId} isCollapsed={!isOpen} {...timeline} clampedScroll={clampedScroll} />
      ))}
      </div>
    </div>
  )
