...
export const generateTimelineThumbnailVideo = async (
    ...
    overHang: number,
    applyMask: (canvas: HTMLCanvasElement, frameIndex?: number) => HTMLCanvasElement,
    ...
    frameIndices = frameIndices.map((frameIndex) => {
        ...
        const mediaStartFrame = Math.round((mediaInfoRef?.startFrame ?? 0) / projectFps * clipFps)
        const mediaEndFrame = Math.round(( mediaInfoRef?.endFrame ?? 0) / projectFps * clipFps)
