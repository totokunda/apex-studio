...
export const SemiGauge: React.FC<SemiGaugeProps> = ({ value, label, size = 84 }) => {
    ...
    <div className="flex flex-col items-center justify-center">
      <svg width={size} height={size / 2} viewBox={`0 0 ${size} ${size / 2}`}>
        ...
        <path
          ...
          strokeWidth={stroke}
          strokeLinecap="round"
          style={{ strokeDasharray: arcLen, strokeDashoffset: arcLen - progressLen, transition: 'stroke-dashoffset 250ms ease, stroke 250ms ease' }}
        />
      </svg>
      <div className="-mt-2.5 text-center">
        <div className="text-[14px] font-bold text-brand-light">{clamped.toFixed(0)}%</div>
        <div className="text-[12px] text-brand-light font-semibold">{label}</div>
      </div>
    </div>
  );
};
