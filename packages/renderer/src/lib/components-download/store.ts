...
export const useComponentsDownloadStore = create<ComponentsDownloadStore>()(
  persist(
    (set, get) => ({
      entries: {},
      jobIndex: {},
      ...
      connectJob: async (jobId: string, path: string) => {
        ...
        let pending: {
          pct: number;
          payload?: ComponentsDownloadPayload;
        } | null = null;
        ...
        const emitNow = (pct: number, payload?: ComponentsDownloadPayload) => {
          ...
          const downloaded =
            ...
            (payload as any)?.downloaded ??
            (payload as any)?.bytes_downloaded;
          const total =
            payload?.metadata?.total ??
            (payload as any)?.total ??
            (payload as any)?.bytes_total;
          ...
          if (filename) {
            ...
            if (
              ...
              if (timeDelta > 0 && bytesDelta > 0)
                fileSpeed = bytesDelta / timeDelta;
            }
            nextFiles[filename] = {
              filename,
              progress: pct,
          ...
          if (fileList.length > 0) {
            const totalsKnown = fileList.every(
              (f) => typeof f.totalBytes === "number" && f.totalBytes! > 0,
            );
            if (totalsKnown) {
              const sumDownloaded = fileList.reduce(
                (s, f) => s + (f.downloadedBytes || 0),
                0,
              );
              const sumTotal = fileList.reduce(
                (s, f) => s + (f.totalBytes || 0),
                0,
              );
              ...
              aggProgress =
                sumTotal > 0
                  ? Math.max(
                      ...
                      Math.min(
                        100,
                        Math.floor((sumDownloaded / sumTotal) * 100),
                      ),
                    )
                  : (pct ?? 0);
            } else {
              const avg =
                fileList.reduce(
                  (s, f) =>
                    s + (typeof f.progress === "number" ? f.progress : 0),
                  0,
                ) / fileList.length;
              aggProgress = isFinite(avg) ? avg : (pct ?? 0);
            }
            // Sum speeds where available
            const sumSpeed = fileList.reduce(
              (s, f) => s + (f.downloadSpeed || 0),
              0,
            );
          ...
          get().setEntry(path, {
            ...
            status: "downloading",
            downloadedBytes:
              aggDownloaded ?? downloaded ?? currentEntry?.downloadedBytes,
        ...
        const applyProgress = (
          ...
          const shouldEmitByStep =
            lastProgress == null ||
            Math.abs(pct - lastProgress) >= MIN_PROGRESS_STEP;
        ...
        const handlePayload = (
          ...
          if (["canceled", "cancelled"].includes(status)) {
            ...
            if (entrySnapshot?.manifestIds?.length) {
              requestManifestSync(entrySnapshot.manifestIds, {
                includeList: true,
              });
            }
            try {
          ...
          if (
            ["completed", "complete", "success", "done", "finished"].includes(
              status,
            )
          ) {
            flushPending();
            ...
            if (filename) {
              ...
              const downloaded =
                prev?.downloadedBytes ?? payload?.metadata?.downloaded ?? total;
              const files = { ...(currentEntry?.files || {}) };
              files[filename] = {
                filename,
                progress: 100,
                ...
                downloadedBytes:
                  typeof downloaded === "number"
                    ...
                    : prev?.downloadedBytes,
                totalBytes:
                  typeof total === "number" ? total : prev?.totalBytes,
                downloadSpeed: 0,
                lastUpdateTime: Date.now(),
                error: undefined,
              };
              ...
              const isModelKind =
                (entrySnapshot?.kind || "unknown") === "model";
              get().setEntry(path, {
                status: "completed",
                progress: 100,
                awaitingManifest: isModelKind,
              });
              if (entrySnapshot?.manifestIds?.length) {
                requestManifestSync(entrySnapshot.manifestIds, {
                  includeList: true,
              ...
              if (!isModelKind) {
                setTimeout(() => {
                  try {
                    ...
                    if (
                      entryNow &&
                      entryNow.jobId === (entrySnapshot?.jobId || jobId)
                    ) {
        ...
        set((s) => ({
          jobIndex: {
            ...s.jobIndex,
            [jobId]: Array.from(new Set([...(s.jobIndex[jobId] || []), path])),
          },
        }));
      },
      ...
      startPath: async (path: string, options?: StartDownloadOptions) => {
        ...
        const manifestIdList = (
          manifestIds && manifestIds.length
            ? manifestIds
            : manifestId
              ? [manifestId]
              : []
        ...
        manifestIdList.forEach((id) => {
          try {
            get().clearPendingDeletion(id, path);
          } catch {}
        });
        set((s) => ({
          entries: {
            ...s.entries,
            [path]: { ...(s.entries[path] || baseEntry), ...baseEntry },
          },
        }));
      ...
      cancelPath: async (path: string, onCanceled?: () => void) => {
        ...
        if (!entry) return;
        try {
          await cancelComponents(entry.jobId);
        } catch {}
        set((s) => ({
          entries: {
            ...s.entries,
            [path]: {
              ...(s.entries[path] as DownloadEntry),
              status: "canceled",
              awaitingManifest: false,
            },
          },
      ...
      clearPendingDeletion: (manifestId: string, path: string) => {
        ...
        set((s) => {
          ...
          if (Object.keys(nextManifestMap).length > 0)
            nextPending[manifestId] = nextManifestMap;
    ...
    }),
    {
      ...
      onRehydrateStorage: () => async (state) => {
        ...
        for (const [path, entry] of Object.entries(entries)) {
          ...
          if (
            entry.manifestIds?.length &&
            (entry.awaitingManifest || entry.status === "completed")
          ) {
            requestManifestSync(entry.manifestIds, { includeList: true });
          }
...
export function requestManifestSync(
  manifestIds?: string | string[],
  options?: ManifestSyncOptions,
) {
  const ids = (
    Array.isArray(manifestIds) ? manifestIds : manifestIds ? [manifestIds] : []
...
async function flushManifestSyncQueue() {
  ...
  manifestSyncInFlight = (async () => {
    ...
    while (manifestSyncQueue.size > 0 || manifestListRefreshRequested) {
      ...
      if (ids.length > 0) {
        await Promise.all(
          ids.map((id) => loadManifest(id, true).catch(() => {})),
        );
      }
      if (refreshList) {
        try {
...
function normalizeModelPaths(
  component: ManifestComponent | Record<string, any>,
): NormalizedPath[] {
  const raw = Array.isArray(component?.model_path)
  ...
  for (const item of raw) {
    if (typeof item === "string") {
      ...
      entries.push({
        path: item.path,
        downloaded:
          typeof item.is_downloaded === "boolean"
            ? !!item.is_downloaded
            : undefined,
      });
    }
...
function normalizeSchedulerConfigs(
  component: ManifestComponent | Record<string, any>,
): NormalizedPath[] {
  const opts = Array.isArray(component?.scheduler_options)
    ? component.scheduler_options
    : [];
  const entries: NormalizedPath[] = [];
  for (const opt of opts as Array<
    ManifestSchedulerOption & { is_downloaded?: boolean }
  >) {
    if (!opt || !opt.config_path) continue;
    entries.push({
      path: String(opt.config_path),
      downloaded:
        typeof opt.is_downloaded === "boolean"
          ? !!opt.is_downloaded
          : undefined,
    });
  }
...
function buildManifestPathStatus(
  manifest: ManifestDocument,
): Map<string, boolean> {
  const map = new Map<string, boolean>();
  const components = Array.isArray(manifest?.spec?.components)
  ...
  >) {
    ...
    const mark = (path: string | undefined, overridden?: boolean) => {
      if (!path) return;
      const flag =
        typeof overridden === "boolean" ? overridden : componentDownloaded;
      map.set(String(path), flag);
    };
    normalizeModelPaths(component).forEach((item) =>
      mark(item.path, item.downloaded),
    );
    if (component?.config_path) mark(String(component.config_path));
    normalizeSchedulerConfigs(component).forEach((item) =>
      mark(item.path, item.downloaded),
    );
  ...
  if (pending) {
    for (const pendingPath of Object.keys(pending)) {
      ...
      if (status === false || status == null) {
        try {
          store.clearPendingDeletion(manifestId, pendingPath);
        } catch {}
      }
    }
  }
