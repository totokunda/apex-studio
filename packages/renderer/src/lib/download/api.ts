...
export async function getUnifiedDownloadStatus(jobId: string): Promise<ConfigResponse<UnifiedJobStatus>> {
    return await getUnifiedDownloadStatusPreload(jobId);
}

export async function cancelUnifiedDownload(jobId: string): Promise<ConfigResponse<{job_id: string; status: string; message?: string}>> {
    return await cancelUnifiedDownloadPreload(jobId) as ConfigResponse<{job_id: string; status: string; message?: string}>;
}

export function onUnifiedDownloadUpdate(jobId: string, callback: (data: UnifiedDownloadWsUpdate) => void): () => void {
    return onUnifiedDownloadUpdatePreload(jobId, callback as unknown as (data: any) => void);
}

export function onUnifiedDownloadStatus(jobId: string, callback: (data: WsConnectionStatus) => void): () => void {
    return onUnifiedDownloadStatusPreload(jobId, callback as unknown as (data: any) => void);
}

export function onUnifiedDownloadError(jobId: string, callback: (data: { error: string; raw?: any }) => void): () => void {
    return onUnifiedDownloadErrorPreload(jobId, callback as unknown as (data: any) => void);
}

export async function connectUnifiedDownloadWebSocket(jobId: string): Promise<ConfigResponse<any>> {
    return await connectUnifiedDownloadWebSocketPreload(jobId);
}

export async function disconnectUnifiedDownloadWebSocket(jobId: string): Promise<ConfigResponse<any>> {
