...
import { startUnifiedDownload, resolveUnifiedDownload, resolveUnifiedDownloadBatch, getUnifiedDownloadStatus, cancelUnifiedDownload, deleteDownload, onUnifiedDownloadUpdate, onUnifiedDownloadStatus, onUnifiedDownloadError, connectUnifiedDownloadWebSocket, disconnectUnifiedDownloadWebSocket, type UnifiedJobStatus, type UnifiedDownloadWsUpdate, type WsConnectionStatus } from './api';

export interface DownloadStore {
  downloadingPaths: Set<string>;
  jobIdToPath: Record<string, string | string[]>;
  wsFilesByPath: Record<string, Record<string, {
    filename: string;
  ...
  resolveDownload: (request: {
    ...
    source: string | string[];
    save_path?: string;
  }) => Promise<{job_id: string; exists: boolean; running: boolean; downloaded: boolean; bucket: string; save_dir: string; source: string | string[]} | undefined>;
  resolveDownloadBatch: (request: {
    item_type: 'component' | 'lora' | 'preprocessor';
    sources: Array<string | string[]>;
    save_path?: string;
  }) => Promise<{results: Array<{job_id: string; exists: boolean; running: boolean; downloaded: boolean; bucket: string; save_dir: string; source: string | string[]}>} | undefined>;
  getDownloadStatus: (jobId: string) => Promise<UnifiedJobStatus | undefined>;
  cancelDownload: (jobId: string) => Promise<{ job_id: string; status: string; message?: string } | undefined>;
  deleteDownload: (request: {
    path: string;
    item_type?: 'component' | 'lora' | 'preprocessor';
    source?: string | string[];
    save_path?: string;
  }) => Promise<{path: string; status: string; removed_mapping?: boolean; unmarked?: boolean} | undefined>;
  connectDownloadWS: (jobId: string) => Promise<boolean>;
  disconnectDownloadWS: (jobId: string) => Promise<boolean>;
  onDownloadUpdate: (jobId: string, cb: (data: UnifiedDownloadWsUpdate) => void) => () => void;
  onDownloadStatus: (jobId: string, cb: (data: WsConnectionStatus) => void) => () => void;
  onDownloadError: (jobId: string, cb: (data: { error: string; raw?: any }) => void) => () => void;
}
...
export const useDownloadStore = create<DownloadStore>()((set, get) => ({
  ...
  subscribeToJob: async (jobId: string, pathOrPaths: string | string[], onComplete?: (arg: any) => void, onError?: (error: unknown, arg: any) => void) => {
    ...
    const off = onUnifiedDownloadUpdate(jobId, (data: UnifiedDownloadWsUpdate) => {
      try {
        ...
        const downloaded = typeof meta.downloaded === 'number' ? (meta.downloaded as number) : undefined;
        const total = typeof meta.total === 'number' ? (meta.total as number) : undefined;
        const label = typeof meta.label === 'string' ? (meta.label as string) : undefined;
        const bucket = (meta.bucket === 'component' || meta.bucket === 'lora' || meta.bucket === 'preprocessor') ? meta.bucket : undefined;
        const progress = typeof (data as any)?.progress === 'number' ? (data as any).progress : null;
        const message = typeof (data as any)?.message === 'string' ? (data as any).message : undefined;
        const status = (data as any)?.status as string | undefined;
        const isTerminal = !!status && (status === 'complete' || status === 'completed' || status === 'error' || status === 'canceled')
        ...
        if (isTerminal) {
          ...
          setTimeout(() => {set((state) => {
            ...
            for (const p of pathsArr) { if (p && nextWsFilesByPath[p]) { delete nextWsFilesByPath[p]; } }
        ...
        if (!isTerminal && primaryPath && filename && (downloaded != null || total != null)) {
          set((state) => {
            const currentForPath = state.wsFilesByPath[primaryPath] || {};
            const prev = currentForPath[filename];
            const now = Date.now();

            const prevDownloaded = typeof prev?.downloadedBytes === 'number'
              ? prev.downloadedBytes
              : downloaded ?? 0;
            const prevTs = typeof prev?.lastUpdateTs === 'number' ? prev.lastUpdateTs : now;
            ...
            if (downloaded != null && dtSeconds > 0.25 && downloaded > prevDownloaded) {
              ...
              wsFilesByPath: {
                ...
                [primaryPath]: {
                  ...
                  [filename]: {
                    ...
                    totalBytes: total ?? (currentForPath[filename]?.totalBytes ?? 0),
                    status,
                    progress,
                    message,
                    bucket,
  ...
  startAndTrackDownload: async (request: {
    item_type: 'component' | 'lora' | 'preprocessor';
    source: string | string[];
    save_path?: string;
    manifest_id?: string;
