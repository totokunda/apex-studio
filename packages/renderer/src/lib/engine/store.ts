...
export interface JobProgress {
  jobId: string;
  status: 'pending' | 'running' | 'complete' | 'failed' | 'connected' | 'queued';
  progress?: number;
  currentStep?: string;
  error?: string;
  result?: JobResult;
...
function normalizeStatus(status: any): JobProgress['status'] {
  try {
    ...
    if (s === 'queued' || s === 'pending' || s === 'waiting' || s === 'unknown') return 'pending';
    if (s === 'processing' || s === 'running' || s === 'in_progress' || s === 'in-progress') return 'running';
    if (s === 'complete' || s === 'completed' || s === 'success' || s === 'succeeded') return 'complete';
    if (s === 'failed' || s === 'error' || s === 'canceled' || s === 'cancelled' || s === 'stopped') return 'failed';
...
export const useEngineJobStore = create<EngineJobStore>()(subscribeWithSelector((set, get) => ({
  jobs: {},
  activeJobs: new Set(),
  ...
  startTracking: async (jobId: string) => {
    ...
    job.onUpdate(async (data) => {
      ...
      const normalizedStatus = normalizeStatus((data as any)?.status || existing?.status || 'running');
      ...
      const normalizedProgress = typeof data?.progress === 'number'
        ? (data.progress <= 1 ? Math.round(data.progress * 100) : Math.round(data.progress))
        : (typeof existing?.progress === 'number' ? Math.round(existing.progress) : undefined);
      const stepMsg = (data?.message || data?.step || existing?.currentStep) as any;
      const nextUpdates = [
  ...
  clearCompletedJobs: () => {
    const { jobs } = get();
    const completed = Object.keys(jobs).filter((id) => jobs[id].status === 'complete' || jobs[id].status === 'failed');
