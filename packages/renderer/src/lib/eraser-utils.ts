 ...
 */
function keepIntervalsForSegment(A: Pt, B: Pt, eraserPts: Pt[], r: number): Array<[number, number]> {
...
export function erasePolylineByEraser(
  ...
  // Convert to Pt[]
  const P: Pt[] = [];
  for (let i = 0; i < points.length; i += 2) P.push({ x: points[i], y: points[i+1] });
  ...
  const pushCurrentIfValid = () => {
    if (current.length >= 2) {
      ...
      for (let i = 1; i < current.length; i++) {
        const a = cleaned[cleaned.length-1], b = current[i];
 ...
 */
export function mergeConnectedLines(lines: any[], aggressive: boolean = false): any[] {
