...
export const useInputControlsStore = create<InputControlStore>((set, get) => ({
  ...
  incrementTotalTimelineFrames: (frames, inputId) => {
    ...
    if (!inputId) {
      set((s) => ({
        totalTimelineFrames: Math.max(1, s.totalTimelineFrames + delta),
      }));
      return;
    }
    set((state) => {
      const current =
        state.totalTimelineFramesByInputId[inputId] ?? defaultTotalFrames;
      return {
        totalTimelineFramesByInputId: {
          ...state.totalTimelineFramesByInputId,
...
function __requestNextInputTick(inputId: string) {
  __inputPlaybackRafIdByInput[inputId] = requestAnimationFrame((now) =>
    __inputPlaybackTick(inputId, now),
  );
}
...
function __inputPlaybackTick(inputId: string, now: number) {
  ...
  const totalFrames = Math.max(
    0,
    controls.getTotalTimelineFrames(inputId) || 0,
  );
  const [rangeStartRaw, rangeEndRaw] = controls.getSelectedRange(inputId) || [
    0, 1,
  ];
  ...
  __inputFrameAccumulatorByInput[inputId] =
    (__inputFrameAccumulatorByInput[inputId] || 0) + dt * fps;
...
useInputControlsStore.setState((prev) => ({
  ...
  play: (inputId: string) => {
    ...
    const [rangeStartRaw, rangeEndRaw] = state.getSelectedRange(inputId) || [
      0, 1,
    ];
    const rangeStart = Math.max(0, Math.round(rangeStartRaw));
    const rangeEndExclusive = Math.max(rangeStart + 1, Math.round(rangeEndRaw));
...
export function updateInputZoomLevel(
  clips: AnyClipProps[],
