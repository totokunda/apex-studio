...
export const getVideoFrameIterator = async (
    ...
    const videoCanBeTransparent = await mediaInfo.video.canBeTransparent();
    const decoderMaybe = getOrCreateVideoDecoder(path, mediaInfo, videoCanBeTransparent, options?.isolationKey);
    ...
    const endIndex = Number.isFinite(options.endIndex as number) ? Math.max(0, Math.floor(options.endIndex as number)) : undefined;
    ...
    async function* iterate(): AsyncGenerator<WrappedCanvas | null> {
        try {
            ...
            const endTimestamp = typeof endIndex === 'number' ? (endIndex / projectFps) * speed : undefined;


            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
            const stream = await (decoder.sink as any).canvases(startTimestamp, endTimestamp);
            const it = (stream as AsyncIterable<WrappedCanvas | null>)[Symbol.asyncIterator]();

            let prev: WrappedCanvas | null = null; // last decoded <= target ts
            let next = await it.next();

            const lastIndex = typeof endIndex === 'number' ? endIndex : Number.POSITIVE_INFINITY;
            for (let i = startIndex; i < lastIndex; i++) {
                const targetTs = (i / projectFps) * speed;
                // Advance until next frame is at or after target timestamp
                while (!next.done && next.value && typeof next.value.timestamp === 'number' && next.value.timestamp < targetTs) {
                    prev = next.value;
                    next = await it.next();
                }

                let out: WrappedCanvas | null = null;
                if (prev && typeof prev.timestamp === 'number' && prev.timestamp <= targetTs) {
