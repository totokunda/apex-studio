...
import { JobResult, connectJobWebSocket as connectWS, disconnectJobWebSocket as disconnectWS, subscribeToJobErrors, subscribeToJobStatus, subscribeToJobUpdates, getPostprocessorStatus } from './api';
...
export const usePostprocessorJobStore = create<PostprocessorJobStore>()(subscribeWithSelector((set, get) => ({
  jobs: {},
  activeJobs: new Set(),
  ...
  startTracking: async (jobId: string) => {
    ...
    // Connect websocket
    try { await connectWS(jobId); } catch {}
  ...
  stopTracking: async (jobId: string) => {
    if (!jobId) return;
    try { await disconnectWS(jobId); } catch {}
    set((state) => {
  ...
  fetchJobResult: async (jobId: string): Promise<boolean> => {
    try {
      ...
      get().updateJobProgress(jobId, { error: res.error || 'Failed to fetch result', lastUpdate: Date.now() });
      return false;
    } catch (e) {
      get().updateJobProgress(jobId, { error: e instanceof Error ? e.message : 'Failed to fetch result', lastUpdate: Date.now() });
      return false;
    }
  },
  ...
  clearCompletedJobs: () => {
    const { jobs } = get();
    const done = Object.keys(jobs).filter((id) => jobs[id].status === 'complete' || jobs[id].status === 'failed');
