...
import {fetchSample, getMediaInfo, prefetchSamples} from '@/lib/media'
...
const usePrefetch = (mediaInfo: MediaInfo | null, framesToPrefetch: number, currentFrame: number, path: string) => {
    ...
    useEffect(() => {
      ...
      (async () => {
        const totalFrames = Math.max(0, (mediaInfo.stats.video?.packetCount || 0) - 1);
        ...
        try {
          await prefetchSamples(path, frameIndices, codedWidth, codedHeight, {mediaInfo});
        } catch (e) {
          console.error(e);
        }
      })();
...
const VideoPreview: React.FC<VideoClipProps & {framesToPrefetch?: number, rectWidth: number, rectHeight: number}> = ({ src, clipId, startFrame = 0, framesToPrefetch = 96, rectWidth, rectHeight}) => {
    ...
    const currentFrame = useMemo(() => focusFrame - startFrame, [focusFrame, startFrame]);
    const tool = useViewportStore((s) => s.tool);
    const scale = useViewportStore((s) => s.scale);
    const position = useViewportStore((s) => s.position);
    const _getClipPosition = useViewportStore((s) => s.getClipPosition);
    const _setClipPosition = useViewportStore((s) => s.setClipPosition);
    const clipPosition = _getClipPosition(clipId);
    const setClipPosition = useCallback((position: Point) => _setClipPosition(clipId, position), [clipId, _setClipPosition]);
    ...
    const updateGuidesAndMaybeSnap = useCallback((opts: { snap: boolean }) => {
        ...
        if (opts.snap) {
            ...
            if (deltaX !== 0 || deltaY !== 0) {
                ...
                setClipPosition({ x: node.x(), y: node.y() });
            }
        }
    }, [rectWidth, rectHeight, scale, setClipPosition, isRotating]);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const info = await getMediaInfo(src.split('/').pop()!);
    ...
    const draw = useCallback(async () => {
        ...
        try {
            ...
            const sample = await fetchSample(src, targetFrame, mediaInfo?.video?.codedWidth, mediaInfo?.video?.codedHeight, {mediaInfo});
            if (!sample) return;
    ...
    }, [setClipPosition, updateGuidesAndMaybeSnap]);
    ...
    const handleDragStart = useCallback((e: Konva.KonvaEventObject<MouseEvent>) => {
        ...
        updateGuidesAndMaybeSnap({ snap: true });
    }, [setClipPosition, clipId, addClipSelection, updateGuidesAndMaybeSnap]);
    ...
    }, [setClipPosition, addClipSelection]);
    ...
    }, [transformerRef.current, updateGuidesAndMaybeSnap]);
